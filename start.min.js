var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.findInternal=function(a,c,b){a instanceof String&&(a=String(a));for(var d=a.length,f=0;f<d;f++){var h=a[f];if(c.call(b,h,f,a))return{i:f,v:h}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(a,c,b){a!=Array.prototype&&a!=Object.prototype&&(a[c]=b.value)};
$jscomp.getGlobal=function(a){return"undefined"!=typeof window&&window===a?a:"undefined"!=typeof global&&null!=global?global:a};$jscomp.global=$jscomp.getGlobal(this);$jscomp.polyfill=function(a,c,b,d){if(c){b=$jscomp.global;a=a.split(".");for(d=0;d<a.length-1;d++){var f=a[d];f in b||(b[f]={});b=b[f]}a=a[a.length-1];d=b[a];c=c(d);c!=d&&null!=c&&$jscomp.defineProperty(b,a,{configurable:!0,writable:!0,value:c})}};
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,b){return $jscomp.findInternal(this,a,b).v}},"es6-impl","es3");$jscomp.polyfill("Object.is",function(a){return a?a:function(a,b){return a===b?0!==a||1/a===1/b:a!==a&&b!==b}},"es6-impl","es3");$jscomp.polyfill("Array.prototype.includes",function(a){return a?a:function(a,b){var c=this;c instanceof String&&(c=String(c));var f=c.length;for(b=b||0;b<f;b++)if(c[b]==a||Object.is(c[b],a))return!0;return!1}},"es7","es3");
$jscomp.checkStringArgs=function(a,c,b){if(null==a)throw new TypeError("The 'this' value for String.prototype."+b+" must not be null or undefined");if(c instanceof RegExp)throw new TypeError("First argument to String.prototype."+b+" must not be a regular expression");return a+""};$jscomp.polyfill("String.prototype.includes",function(a){return a?a:function(a,b){return-1!==$jscomp.checkStringArgs(this,a,"includes").indexOf(a,b||0)}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var f=c.length,h=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var k=0;k<h&&b<f;)if(c[b++]!=a[k++])return!1;return k>=h}},"es6-impl","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};$jscomp.symbolCounter_=0;
$jscomp.Symbol=function(a){return $jscomp.SYMBOL_PREFIX+(a||"")+$jscomp.symbolCounter_++};$jscomp.initSymbolIterator=function(){$jscomp.initSymbol();var a=$jscomp.global.Symbol.iterator;a||(a=$jscomp.global.Symbol.iterator=$jscomp.global.Symbol("iterator"));"function"!=typeof Array.prototype[a]&&$jscomp.defineProperty(Array.prototype,a,{configurable:!0,writable:!0,value:function(){return $jscomp.arrayIterator(this)}});$jscomp.initSymbolIterator=function(){}};
$jscomp.arrayIterator=function(a){var c=0;return $jscomp.iteratorPrototype(function(){return c<a.length?{done:!1,value:a[c++]}:{done:!0}})};$jscomp.iteratorPrototype=function(a){$jscomp.initSymbolIterator();a={next:a};a[$jscomp.global.Symbol.iterator]=function(){return this};return a};$jscomp.makeIterator=function(a){$jscomp.initSymbolIterator();var c=a[Symbol.iterator];return c?c.call(a):$jscomp.arrayIterator(a)};$jscomp.owns=function(a,c){return Object.prototype.hasOwnProperty.call(a,c)};
$jscomp.polyfill("WeakMap",function(a){function c(a){$jscomp.owns(a,d)||$jscomp.defineProperty(a,d,{value:{}})}function b(a){var b=Object[a];b&&(Object[a]=function(a){c(a);return b(a)})}if(function(){if(!a||!Object.seal)return!1;try{var b=Object.seal({}),A=Object.seal({}),c=new a([[b,2],[A,3]]);if(2!=c.get(b)||3!=c.get(A))return!1;c.delete(b);c.set(A,4);return!c.has(b)&&4==c.get(A)}catch(F){return!1}}())return a;var d="$jscomp_hidden_"+Math.random().toString().substring(2);b("freeze");b("preventExtensions");
b("seal");var f=0,h=function(a){this.id_=(f+=Math.random()+1).toString();if(a){$jscomp.initSymbol();$jscomp.initSymbolIterator();a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};h.prototype.set=function(a,b){c(a);if(!$jscomp.owns(a,d))throw Error("WeakMap key fail: "+a);a[d][this.id_]=b;return this};h.prototype.get=function(a){return $jscomp.owns(a,d)?a[d][this.id_]:void 0};h.prototype.has=function(a){return $jscomp.owns(a,d)&&$jscomp.owns(a[d],this.id_)};h.prototype.delete=
function(a){return $jscomp.owns(a,d)&&$jscomp.owns(a[d],this.id_)?delete a[d][this.id_]:!1};return h},"es6-impl","es3");$jscomp.MapEntry=function(){};
$jscomp.polyfill("Map",function(a){if(!$jscomp.ASSUME_NO_NATIVE_MAP&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([[b,"s"]]));if("s"!=c.get(b)||1!=c.size||c.get({x:4})||c.set({x:4},"t")!=c||2!=c.size)return!1;var d=c.entries(),f=d.next();if(f.done||f.value[0]!=b||"s"!=f.value[1])return!1;f=d.next();return f.done||4!=f.value[0].x||"t"!=f.value[1]||!d.next().done?!1:!0}catch(K){return!1}}())return a;$jscomp.initSymbol();
$jscomp.initSymbolIterator();var c=new WeakMap,b=function(a){this.data_={};this.head_=h();this.size=0;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};b.prototype.set=function(a,b){var c=d(this,a);c.list||(c.list=this.data_[c.id]=[]);c.entry?c.entry.value=b:(c.entry={next:this.head_,previous:this.head_.previous,head:this.head_,key:a,value:b},c.list.push(c.entry),this.head_.previous.next=c.entry,this.head_.previous=c.entry,this.size++);return this};b.prototype.delete=
function(a){a=d(this,a);return a.entry&&a.list?(a.list.splice(a.index,1),a.list.length||delete this.data_[a.id],a.entry.previous.next=a.entry.next,a.entry.next.previous=a.entry.previous,a.entry.head=null,this.size--,!0):!1};b.prototype.clear=function(){this.data_={};this.head_=this.head_.previous=h();this.size=0};b.prototype.has=function(a){return!!d(this,a).entry};b.prototype.get=function(a){return(a=d(this,a).entry)&&a.value};b.prototype.entries=function(){return f(this,function(a){return[a.key,
a.value]})};b.prototype.keys=function(){return f(this,function(a){return a.key})};b.prototype.values=function(){return f(this,function(a){return a.value})};b.prototype.forEach=function(a,b){for(var c=this.entries(),d;!(d=c.next()).done;)d=d.value,a.call(b,d[1],d[0],this)};b.prototype[Symbol.iterator]=b.prototype.entries;var d=function(a,b){var d=b&&typeof b;"object"==d||"function"==d?c.has(b)?d=c.get(b):(d=""+ ++k,c.set(b,d)):d="p_"+b;var f=a.data_[d];if(f&&$jscomp.owns(a.data_,d))for(a=0;a<f.length;a++){var h=
f[a];if(b!==b&&h.key!==h.key||b===h.key)return{id:d,list:f,index:a,entry:h}}return{id:d,list:f,index:-1,entry:void 0}},f=function(a,b){var c=a.head_;return $jscomp.iteratorPrototype(function(){if(c){for(;c.head!=a.head_;)c=c.previous;for(;c.next!=c.head;)return c=c.next,{done:!1,value:b(c)};c=null}return{done:!0,value:void 0}})},h=function(){var a={};return a.previous=a.next=a.head=a},k=0;return b},"es6-impl","es3");
$jscomp.polyfill("Set",function(a){if(!$jscomp.ASSUME_NO_NATIVE_SET&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([b]));if(!c.has(b)||1!=c.size||c.add(b)!=c||1!=c.size||c.add({x:4})!=c||2!=c.size)return!1;var f=c.entries(),h=f.next();if(h.done||h.value[0]!=b||h.value[1]!=b)return!1;h=f.next();return h.done||h.value[0]==b||4!=h.value[0].x||h.value[1]!=h.value[0]?!1:f.next().done}catch(k){return!1}}())return a;
$jscomp.initSymbol();$jscomp.initSymbolIterator();var c=function(a){this.map_=new Map;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)this.add(b.value)}this.size=this.map_.size};c.prototype.add=function(a){this.map_.set(a,a);this.size=this.map_.size;return this};c.prototype.delete=function(a){a=this.map_.delete(a);this.size=this.map_.size;return a};c.prototype.clear=function(){this.map_.clear();this.size=0};c.prototype.has=function(a){return this.map_.has(a)};c.prototype.entries=function(){return this.map_.entries()};
c.prototype.values=function(){return this.map_.values()};c.prototype.keys=c.prototype.values;c.prototype[Symbol.iterator]=c.prototype.values;c.prototype.forEach=function(a,c){var b=this;this.map_.forEach(function(d){return a.call(c,d,d,b)})};return c},"es6-impl","es3");
$jscomp.polyfill("Array.from",function(a){return a?a:function(a,b,d){$jscomp.initSymbolIterator();b=null!=b?b:function(a){return a};var c=[],h=a[Symbol.iterator];if("function"==typeof h)for(a=h.call(a);!(h=a.next()).done;)c.push(b.call(d,h.value));else for(var h=a.length,k=0;k<h;k++)c.push(b.call(d,a[k]));return c}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.endsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"endsWith");a+="";void 0===b&&(b=c.length);b=Math.max(0,Math.min(b|0,c.length));for(var f=a.length;0<f&&0<b;)if(c[--b]!=a[--f])return!1;return 0>=f}},"es6-impl","es3");
var saveAs=saveAs||function(a){if("undefined"===typeof navigator||!/MSIE [1-9]\./.test(navigator.userAgent)){var c=a.document.createElementNS("http://www.w3.org/1999/xhtml","a"),b="download"in c,d=/Version\/[\d\.]+.*Safari/.test(navigator.userAgent),f=a.webkitRequestFileSystem,h=a.requestFileSystem||f||a.mozRequestFileSystem,k=function(b){(a.setImmediate||a.setTimeout)(function(){throw b;},0)},A=0,J=function(b){var c=function(){"string"===typeof b?(a.URL||a.webkitURL||a).revokeObjectURL(b):b.remove()};
a.chrome?c():setTimeout(c,500)},F=function(a,b,c){b=[].concat(b);for(var d=b.length;d--;){var f=a["on"+b[d]];if("function"===typeof f)try{f.call(a,c||a)}catch(n){k(n)}}},G=function(a){return/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(a.type)?new Blob(["\ufeff",a],{type:a.type}):a},K=function(k,u,H){H||(k=G(k));var m=this;H=k.type;var L=!1,n=function(){F(m,["writestart","progress","write","writeend"])},q=function(){if(e&&d&&"undefined"!==typeof FileReader){var b=
new FileReader;b.onloadend=function(){var a=b.result;e.location.href="data:attachment/file"+a.slice(a.search(/[,;]/));m.readyState=m.DONE;n()};b.readAsDataURL(k);m.readyState=m.INIT}else{if(L||!y)y=(a.URL||a.webkitURL||a).createObjectURL(k);e?e.location.href=y:void 0==a.open(y,"_blank")&&d&&(a.location.href=y);m.readyState=m.DONE;n();J(y)}},x=function(a){return function(){if(m.readyState!==m.DONE)return a.apply(this,arguments)}},z={create:!0,exclusive:!1};m.readyState=m.INIT;u||(u="download");if(b){var y=
(a.URL||a.webkitURL||a).createObjectURL(k);setTimeout(function(){c.href=y;c.download=u;var a=new MouseEvent("click");c.dispatchEvent(a);n();J(y);m.readyState=m.DONE})}else{if(a.chrome&&H&&"application/octet-stream"!==H){var r=k.slice||k.webkitSlice;k=r.call(k,0,k.size,"application/octet-stream");L=!0}f&&"download"!==u&&(u+=".download");if("application/octet-stream"===H||f)var e=a;h?(A+=k.size,h(a.TEMPORARY,A,x(function(a){a.root.getDirectory("saved",z,x(function(a){var b=function(){a.getFile(u,z,
x(function(a){a.createWriter(x(function(b){b.onwriteend=function(b){e.location.href=a.toURL();m.readyState=m.DONE;F(m,"writeend",b);J(a)};b.onerror=function(){var a=b.error;a.code!==a.ABORT_ERR&&q()};["writestart","progress","write","abort"].forEach(function(a){b["on"+a]=m["on"+a]});b.write(k);m.abort=function(){b.abort();m.readyState=m.DONE};m.readyState=m.WRITING}),q)}),q)};a.getFile(u,{create:!1},x(function(a){a.remove();b()}),x(function(a){a.code===a.NOT_FOUND_ERR?b():q()}))}),q)}),q)):q()}},
u=K.prototype;if("undefined"!==typeof navigator&&navigator.msSaveOrOpenBlob)return function(a,b,c){c||(a=G(a));return navigator.msSaveOrOpenBlob(a,b||"download")};u.abort=function(){this.readyState=this.DONE;F(this,"abort")};u.readyState=u.INIT=0;u.WRITING=1;u.DONE=2;u.error=u.onwritestart=u.onprogress=u.onwrite=u.onabort=u.onerror=u.onwriteend=null;return function(a,b,c){return new K(a,b,c)}}}("undefined"!==typeof self&&self||"undefined"!==typeof window&&window||this.content);
"undefined"!==typeof module&&module.exports?module.exports.saveAs=saveAs:"undefined"!==typeof define&&null!==define&&null!=define.amd&&define([],function(){return saveAs});
$(function(){function a(a){for(var g="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(g+=String.fromCharCode(64+b));$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".ebenenAnzeige").each(function(){var p=$(this).attr("aktiv");if("plus"==a)for(var b=0;b<g.length;b++)p.includes(g.charAt(b))||(p+=g.charAt(b));else if("minus"==a){for(var P="",b=0;b<p.length;b++)g.includes(p.charAt(b))||(P+=p.charAt(b));p=P}else"gleich"==a&&(p=g);$(this).attr("aktiv",p);b=G(p);$(this).parent().prev(".tshareElement").attr("sb",
p);$(this).wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove()})})}function c(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv").addClass("aktiv")})}function b(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+
parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv")})}function d(){for(var a=Array(27),p=Array(27),b=0;27>b;b++)a[b]=0,p[b]=0;$(".ebenenAnzeige").each(function(){if(!$(this).parent().hasClass("versteckt")){for(var g=$(this).attr("aktiv").toUpperCase(),b=0;b<g.length;b++)p[g.charCodeAt(b)-64]++;if($(this).parent().find(".markieren").hasClass("aktiv"))for(b=0;b<g.length;b++)a[g.charCodeAt(b)-64]++}});return[a,p]}function f(a){var g=!1;a.toUpperCase().startsWith("TAGS_")&&(a=a.split("_")[1],
""==a&&(g=!0),$(".tshareElement").each(function(){var p=$(this).attr("sb");if(null==p||""==p)p="@";for(var b=-1,c=0;c<a.length;c++)p.includes(a.charAt(c))&&(b=c,c=a.length);0<=b||1==g?($(this).attr("style","display:true;"),$(this).next().attr("style","display:true"),$(this).removeClass("versteckt"),$(this).next().removeClass("versteckt"),$(this).next().find(".markieren").removeClass("versteckt")):($(this).attr("style","display:none;"),$(this).next().attr("style","display:none"),$(this).removeClass("versteckt").addClass("versteckt"),
$(this).next().removeClass("versteckt").addClass("versteckt"),$(this).next().find(".markieren").removeClass("versteckt").addClass("versteckt"))}),k(),$("#hideShow").removeClass("komplett").addClass("komplett"))}function h(){$(".tshareElement").each(function(){var a=parseInt($(this).attr("sichtbarkeit"));0==O[a]?$(this).attr("visibility","hidden"):$(this).removeAttr("visibility")})}function k(){for(var a=d(),p=a[0],a=a[1],b=0;27>b;b++)0<a[b]&&p[b]==a[b]?($("[wahl='"+b+"']").removeClass("komplett").addClass("komplett"),
$("[wahl='"+b+"']").attr("style","display:true")):0<a[b]&&p[b]<a[b]?($("[wahl='"+b+"']").removeClass("komplett"),$("[wahl='"+b+"']").attr("style","display:true")):0==a[b]&&($("[wahl='"+b+"']").removeClass("komplett"),$("[wahl='"+b+"']").attr("style","display:none"))}function A(){$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuTop");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");
$("#content").addClass("moveDown");$("[position='top']").hide();$("[position='right']").show();$("[position='left']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu")}function J(){var a=new Date,b=a.getMonth()+1,c="";10>b&&(c="0");var c=c+b,I=a.getDate(),b="";10>I&&(b="0");var b=b+I,I="",t=a.getHours();10>t&&(I="0");var I=I+t,t="",v=a.getMinutes();10>v&&(t="0");var t=t+v,v="",d=a.getSeconds();10>d&&(v="0");v+=d;return""+(1900+a.getYear())+c+b+"_"+I+t+v}function F(a){$("#einsetzen").html("");
new QRCode($("#einsetzen")[0],{text:$("#qrLink").val(),width:parseInt($("#qrKante").val()),height:parseInt($("#qrKante").val()),colorDark:$("#qrFarbe").val(),colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H});var b="";1==$("#qrTextZeigen").prop("checked")&&(b=$("#qrText").val());1==a&&$("#einsetzen").after(D);$("#einsetzen").append("<div class='qrinfo' qrtext='"+$("#qrLink").val()+"' qrcapture='"+b+"' qrsize='"+parseInt($("#qrKante").val())+"' qrfarbe='"+$("#qrFarbe").val()+"'></div><div class='qrcenter'>"+
b+"</div>");$("#einsetzen").removeAttr("id");$(".transparent").remove();k()}function G(a){var b=document.createElement("canvas");a=a.toUpperCase();var g=b.getContext("2d");b.height=35;b.width=106;g.font="10px Sans-Serif";g.fillStyle="#ffff00";g.fillStyle="#000000";for(var c=0;27>c;c++)a.includes(String.fromCharCode(64+c))?(g.fillStyle="#ffffaa",g.fillRect(c%9*12,12*parseInt(c/9),12,12),g.fillStyle="#000000"):g.fillStyle="#aaaaaa",g.fillText(String.fromCharCode(64+c),2+c%9*12,10+12*parseInt(c/9));
g="";for(c=0;c<a.length;c++)g.includes(a.charAt(c))||(g+=a.charAt(c));return"<span class='ebenenAnzeige' aktiv='"+g+"'><img src='"+b.toDataURL()+"'   ></img></span>"}function K(a){for(var b="<div>",g=0;27>g;g++){var c="";a.includes(String.fromCharCode(64+g))&&(c="aktiv");b+="<button id='bs_"+g+"' class='ebenenknopf "+c+"'>"+String.fromCharCode(64+g)+"</button>";0==(g+1)%9&&(b+="<br>")}return b+"</div>"}function u(a){for(var b="",g=".katex .delimsizing.size1;.katex .delimsizing.size2;.katex .delimsizing.size3;.katex .delimsizing.size4;.katex .delimsizing.mult .delim-size1 > span;.katex .delimsizing.mult .delim-size4 > span;.katex .op-symbol.small-op;.katex .op-symbol.large-op;.katex .mathit;.katex .mathbf;.katex .amsrm;.katex .mathbb;.katex .mathcal;.katex .mathfrak;.katex .mathtt;.katex .mathscr;.katex .mathsf;.katex .mainit".split(";"),
b="KaTeX_Size1 KaTeX_Size2 KaTeX_Size3 KaTeX_Size4 KaTeX_Size1 KaTeX_Size4 KaTeX_Size1 KaTeX_Size2 KaTeX_Main KaTeX_Main KaTeX_Math KaTeX_Main KaTeX_AMS KaTeX_AMS KaTeX_Caligraphic KaTeX_Fraktur KaTeX_Typewriter KaTeX_Script KaTeX_SansSerif KaTeX_Main".split(" "),c=0;c<g.length;c++){var t=0;$(g[c]).each(function(){var b=!1;$(this).parents(".tshareElement").next().find(".markieren").each(function(){b=$(this).hasClass("aktiv")});1!=b&&1!=a||t++});g[c]=t}for(var v=new Set,c=0;c<g.length;c++)0<g[c]&&
v.add(b[c]);0<v.size&&(v.add("KaTeX_Main"),v.add("KaTeX_Math"));g=Array.from(v);console.log("Schriften: ");for(var v="",d=$("#katexStyle").text().split("/* KateX_Fonts END */"),b=d[0].split("/**/"),c=0;c<g.length;c++)for(var e=g[c],f=0;f<b.length;f++)-1!=b[f].indexOf(e)&&(v+=b[f].trim());return b=v+d[1]}function L(a){var b=$("#dateiname").val();null!=b&&""==b&&(b="Slideshow");var g=$("#schriftgroesse").val();$("#pathText");var c=$("#checkPath").prop("checked"),t=$("#pathText").val(),v="";1==c&&(v=
v+("<link rel='stylesheet' href='"+t+"/css/style.css'>\n")+("<link rel='stylesheet' href='"+t+"/css/font-awesome.css'>\n")+("<link rel='stylesheet' href='"+t+"/css/katex.css'>\n")+("<link rel='stylesheet' href='"+t+"/css/reveal.css'>\n")+("<link rel='stylesheet' href='"+t+"/css/serif.css'>\n")+("<link rel='stylesheet' href='"+t+"/css/prism.css'>\n")+("<script src='"+t+"/js/reveal.js'>\x3c/script>\n"));t=J();b=b+"_"+t+".html";$(".transparent").remove();var d="",e="",f="",k=0,l=0,q=0,m=0,n=!1,h=0;$(".tshareElement").each(function(){q++;
$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&m++})});0<q&&0==m&&(n=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",g=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){a=$(this).hasClass("aktiv")});var c="=";$(this).next().find(".folientyp").each(function(){c=$(this).val()});1==n&&(a=!0);1==a&&("="==c?(f=e="",0==h&&(e="<section>jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg",
h=1)):"+"==c?(0==h?e="<section>jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg":1==h?(d=d.replace("jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg",""),e="</section><section>jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg"):2==h&&(e="</section></section><section>jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg"),f="",h=1):"++"==c?(0==h?e="<section><section>":1==h?(d=d.replace("jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg","<section>"),e="</section><section>"):2==h&&(e="</section><section>"),h=2,f=""):"/"==c&&(e="<span class='fragment'>",
f="</span>"));1==a&&($(this).hasClass("zeichenflaeche")?d+=e+"<div "+b+g+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n"+f:$(this).hasClass("markdown")?(d+=e+"<div "+b+g+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n"+f,k++):$(this).hasClass("datei")?(l++,d+=e+"<div "+b+g+" class='datei tshareElement'>"+$(this).html()+"</div>\n"+f):$(this).hasClass("toc")?(d+=e+"<div "+b+g+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n"+
f,k++):$(this).hasClass("qrcode")&&(d+=e+"<div "+b+g+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n"+f,k++))});1==h?d+="</section>":2==h&&(d+="</section></section>");var d=d.replace("jafijf\u00dfjw\u00dfmfw829384u\u00df8wfg",""),r=t="";0<k?(t=u(n),r=r=$("#prism").text()+$("#fontawesome").text()):0<l&&(r=r=$("#prism").text()+$("#fontawesome").text());0==a?(b=window.open(),b.document.open(),b.document.write("<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+
window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style> "+t+r+window.atob("")+
"\n"+window.atob("")+
"\nhtml{font-size:"+g+"px;}\n.reveal{font-size:"+g+"px;}\n</style></head><body><div class='reveal'><div class='slides'>"+d+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>")):(g=0==c?new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+t+r+window.atob("")+
window.atob("")+
"\nhtml{font-size:"+g+"px;}\n.reveal{font-size:"+g+"px;}\n</style></head><body><div class='reveal'><div class='slides'>"+d+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>"],{type:"text/html;charset=utf-8"}):new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>\n"+v+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>html{font-size:"+g+"px;}\n.reveal{font-size:"+g+"px;}\n</style></head><body><div class='reveal'><div class='slides'>"+d+"</div></div><script>Reveal.initialize();\x3c/script></body></html>"],{type:"text/html;charset=utf-8"}),saveAs(g,b))}function N(){$(".tshareElement").each(function(){var a=$(this).attr("ft");if(null==a||""==a)a="=";$(this).next().find(".folientyp").val(a)})}function H(){var a="";m();$(".markieren").each(function(b,g){if($(this).hasClass("aktiv")){b=$(this).parent().prev();
var c=$(this).parent().prev().attr("sb");g=$(this).parent().outerHTML();if(null==c||""==c)c="@";var p=$(this).parent().prev().attr("ft");if(null==p||""==p)p="=";c=" sb='"+c+"' ft='"+p+"'";b.hasClass("markdown")?(klasse="markdown adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+g):b.hasClass("zeichenflaeche")?(klasse="zeichenflaeche zeichenflaecheKlick",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+g):b.hasClass("datei")?(klasse="datei",a+="<div class='tshareElement "+
klasse+"'"+c+">"+b.html()+"</div>"+g):b.hasClass("toc")?(klasse="toc adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+g):b.hasClass("qrcode")&&(klasse="qrcode adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+g)}});M=a;return a="JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+a}function m(){1==x&&("zeichenflaeche"==z?(QreatorBezier.showSVG("zeichenflaeche"),x=!1,z=""):"markdown"==z&&(Textabschnitt.zeigeMarkdown("#markdownTable"),x=!1,$("#markdown").addClass("adoccss"),
$("#markdown").removeAttr("id"),z=""))}A();var O=[1,0,1,1,1,1,1,1,1,1],n=998,q=701,x=!1,z="",y="",r="",e=0,l=0,w=!1,M="",C=!1,E="";$(document).on("click",".wahl",function(){m();$(this).hasClass("komplett")?(b($(this).attr("wahl")),$(this).removeClass("komplett")):(c($(this).attr("wahl")),$(this).removeClass("komplett"),$(this).addClass("komplett"));k()});$(document).on("click","#ebenenzuweisung",function(){$("body").append("<div class='transparent'></div>");var a=K("");$(".transparent").append("<div class='fenster'>"+
a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneZuweisungPlus'>Tags hinzuf\u00fcgen</button><button id='ebeneZuweisungMinus'>Tags abziehen</button><button id='ebeneZuweisungGleich'>Tags \u00fcbernehmen</button></br><button id='ebeneAbbrechen'>Abbrechen</button></div>")});$(document).on("click","#ebeneZuweisungPlus",function(){a("plus");k();$(".transparent").remove()});
$(document).on("click","#ebeneZuweisungMinus",function(){a("minus");k();$(".transparent").remove()});$(document).on("click","#ebeneZuweisungGleich",function(){a("gleich");k();$(".transparent").remove()});for(var E="<select class='sichtbarkeit'>",B=0;27>B;B++)E=0==B?E+("<option selected sichtbarkeit='"+B+"'>"+String.fromCharCode(64+B)+"</option> "):E+("<option sichtbarkeit='"+B+"'>"+String.fromCharCode(64+B)+"</option> ");E+="</select>";$(document).on("change",".folientyp",function(a){a.preventDefault();
a=$(this).val();$(this).parent().prev(".tshareElement").attr("ft",a)});$(document).on("change",".auswahlebene",function(a){a.preventDefault();var b=parseInt($(this).prop("selectedIndex"));0<b&&$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".sichtbarkeit").val(""+String.fromCharCode(64+b))});$(this).val("-");k()});$(window).on("hashchange",function(){var a=location.hash.substring(1).toUpperCase();f(a)});$(document).on("change",".sichtbarkeit",function(a){a.preventDefault();
a=parseInt($(this).prop("selectedIndex"));$(this).parent().prev().attr("sichtbarkeit",a);h();k()});var B=G("@"),D="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select>"+B+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button><button class='unten imageQR'></button></span>",
Q="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select><button class='imagePen editPart oben'></button>"+B+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button><button class='unten imageQR'></button></span>";
$(document).on("click","#hideShow",function(){m();$(this).hasClass("komplett")?($(".tshareElement").each(function(){$(this).attr("style","display:true");$(this).removeClass("versteckt")}),$(".menuBar").each(function(){$(this).attr("style","display:true");$(this).removeClass("versteckt");$(this).find(".markieren").removeClass("versteckt")}),$(this).removeClass("komplett"),location.hash=""):($(".markieren").each(function(){$(this).hasClass("aktiv")||($(this).parent().prev(".tshareElement").attr("style",
"display:none"),$(this).parent().attr("style","display:none"),$(this).parent().removeClass("versteckt").addClass("versteckt"),$(this).removeClass("versteckt").addClass("versteckt"),$(this).parent().prev(".tshareElement").removeClass("versteckt").addClass("versteckt"))}),$(this).addClass("komplett"));k()});$(document).ready(function(){jQuery.fn.outerHTML=function(a){return a?this.before(a).remove():jQuery("<p>").append(this.eq(0).clone()).html()};new Clipboard("#kopieren",{text:function(a){return H()}});
new Clipboard(".kopiereAbschnitt",{text:function(a){m();var b="",g=$(a).parent().prev(),c=$(a).parent().outerHTML(),p=g.attr("sb");if(null==p||""==p)p="@";var d=g.attr("ft");if(null==d||""==d)d="=";p=" sb='"+p+"' ft='"+d+"' ";g.hasClass("markdown")?b="<div class='tshareElement markdown adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c:g.hasClass("zeichenflaeche")?b="<div class='tshareElement zeichenflaeche zeichenflaecheKlick'"+p+">"+$(a).parent().prev().html()+"</div>"+c:g.hasClass("datei")?
b="<div class='tshareElement datei'"+p+">"+$(a).parent().prev().html()+"</div>"+c:g.hasClass("toc")?b="<div class='tshareElement toc adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c:g.hasClass("qrcode")&&(b="<div class='tshareElement qrcode adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c);M=b;return"JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+b}});E="";for(var a=0;27>a;a++)E+="<button class='wahl allgemein knopfStandard' wahl='"+a+"' >"+String.fromCharCode(64+a)+"</button>";$("#ebenenwahl").html(E);
$(".tshareElement").each(function(){var a=$(this).attr("ft");if(null==a||""==a)a="=";$(this).next().find(".folientyp").val(a)});k()});window.addEventListener("beforeunload",function(a){return a.returnValue="Eventuelle \u00c4nderungen am Dokument k\u00f6nnten verloren gehen!"});document.addEventListener("paste",function(a){if(1==$("#rahmen").length&&0==$(".transparent").length)if(a=a.clipboardData.items)for(var b=0;b<a.length;b++){var g=a[b];if(g.type.startsWith("image/")){var g=g.getAsFile(),c=new FileReader;
c.onload=function(a){r=new Image;r.onload=function(){l=e=0;w=!0;this.width>this.height&&(w=!1);1==w?(l=Math.min(q,this.height),l<q&&(l=q),e=this.width*l/this.height,e>n&&(l=l*n/e,e=n,w=!1)):(e=Math.min(n,this.width),e<n&&(e=n),l=this.height*e/this.width,l>q&&(e=e*q/l,l=q,w=!0));$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><div id='parameter'></div><button id='abbrechenBild'>Abbrechen</button></div>");0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+
parseInt(100*e/n)+"'></input><span id='breite'>"+parseInt(100*e/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+parseInt(100*l/q)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*l/q)+"'></input><span id='hoehe'>"+parseInt(100*l/q)+"</span>%, Breite: <span id='breite'>"+parseInt(100*e/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,e,l,!1)};r.src=a.target.result};
c.onerror=function(a){console.log(a.target.error.name)};c.onprogress=function(a){console.log(a.loaded,a.total)};c.readAsDataURL(g)}}else console.log("fileList is null.")},!1);$(document).on("click",".imageQR",function(){$("body").append("<div class='transparent'></div>");$(this).parent().after("<div id='einsetzen' class='tshareElement qrcode adoccss' sb='@' ft='='></div>");$(".transparent").append("<div class='fenster'>QR-Code<p>Zu codierender Text:<br> <textarea  cols='80' rows='8' id='qrLink' ></textarea><br><button id='qrdieseSeite'>Aktuelle Seite verwenden</button><p><input type='checkbox' id='qrTextZeigen'>Bildunterschrift:</input><br><input type='text' size='80' id='qrText' ></input><br><p>Kantenl\u00e4nge in Pixel:<br><input type='text' size='15' id='qrKante' value='250'></input><p>Farbe:<br><input id='qrFarbe'  type='color' value='#000000' list='color' /><datalist id='color'><option>#000000</option><option>#ff0000</option><option>#0000ff</option><option>#ff9900</option><option>#008000</option><option>#a52a2a</option><option>#999999</option></datalist><br><p><button id='qrfertig'>OK</button><button id='qrabbrechen'>Abbrechen</button></div>")});
$(document).on("click","#qrabbrechen",function(){$("#einsetzen").remove();$(".transparent").remove()});$(document).on("click","#qrabbrechen2",function(){$("#ersetzen").removeAttr("id");$(".transparent").remove()});$(document).on("click","#qrdieseSeite",function(){var a=location.href;$("#qrLink").val(a);$("#qrText").val(a)});$(document).on("click","#qrfertig",function(){F(!0)});$(document).on("click","#qrfertig2",function(){F(!1)});$(document).on("click",".qrcode",function(){var a=$(this).find(".qrinfo").attr("qrtext"),
b=$(this).find(".qrinfo").attr("qrcapture"),c=$(this).find(".qrinfo").attr("qrsize"),d=$(this).find(".qrinfo").attr("qrfarbe");$(this).attr("id","einsetzen");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>QR-Code<p>Zu codierender Text:<br> <textarea  cols='80' rows='8' id='qrLink' ></textarea><br><button id='qrdieseSeite'>Aktuelle Seite verwenden</button><p><input type='checkbox' id='qrTextZeigen'>Bildunterschrift:</input><br><input type='text' size='80' id='qrText' ></input><br><p>Kantenl\u00e4nge in Pixel:<br><input type='text' size='15' id='qrKante' value='250'></input><p>Farbe:<br><input id='qrFarbe'  type='color' value='#000000' list='color' /><datalist id='color'><option>#000000</option><option>#ff0000</option><option>#0000ff</option><option>#ff9900</option><option>#008000</option><option>#a52a2a</option><option>#999999</option></datalist><br><p><button id='qrfertig2'>OK</button><button id='qrabbrechen2'>Abbrechen</button></div>");
$("#qrLink").val(a);$("#qrText").val(b);""!=b.trim()&&$("#qrTextZeigen").prop("checked",!0);$("#qrKante").val(c);$("#qrFarbe").val(d)});$("#speichern").click(function(){m();$("body").append("<div class='transparent'></div>");var a=y;""==y&&(a="Notiz");$(".transparent").append("<div class='fenster'>Speichern der aktuellen Datei (Zeitstempel und Endung werden angeh\u00e4ngt)<br>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><br><input type='checkbox' id='checkEditor'>Editor in Datei einbetten</input><br><input type='checkbox' id='checkPath'>Pfad zu Bibliotheken ber\u00fccksichtigen: </input><input type='text' id='pathText' value='../assets'></input><br><button id='speichereDatei'>Speichern</button><button id='abbrechen'>Abbrechen</button></div>")});
$("#slideshow").click(function(){m();$("body").append("<div class='transparent'></div>");var a=y;""==y&&(a="Slideshow");$(".transparent").append("<div class='fenster'>Speichern / Zeigen der aktuellen Slideshow (Zeitstempel und Endung werden beim Speichern angeh\u00e4ngt)<p>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><p>Schriftgr\u00f6\u00dfe Text in Pr\u00e4sentation:<br><input type='text' id='schriftgroesse' value='40'></input><p><input type='checkbox' id='checkPath'>Pfad zu Bibliotheken ber\u00fccksichtigen: </input><input type='text' id='pathText' value='../assets'></input><p><button id='speichereSlideshow'>Speichern</button><button id='zeigeSlideshow'>Zeigen</button><button id='abbrechen'>Abbrechen</button></div>")});
$("#menuansicht").click(function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>Position des Men\u00fcs w\u00e4hlen:<br><button class='menu-position' position='left'>Men\u00fc links</button><button class='menu-position' position='top'>Men\u00fc oben</button><button class='menu-position' position='bottom'>Men\u00fc unten</button><button class='menu-position' position='right'>Men\u00fc rechts</button><br><button id='abbrechen'>Abbrechen</button></div>");
$(".fenster").append('<p>Version: 1.0 2017_08_03 12:22<p>Info:<br><div>Icons made by <a href="http://www.freepik.com" title="Freepik">Freepik</a>, <a href="http://www.flaticon.com/authors/yannick" title="Yannick">Yannick</a>, <a href="http://www.flaticon.com/authors/picol" title="Picol">Picol</a>, <a href="http://www.flaticon.com/authors/situ-herrera" title="Situ Herrera">Situ Herrera</a>, <a href="https://www.flaticon.com/authors/google" title="Google">Google</a>, <a href="http://www.flaticon.com/authors/freepik">freepik</a>, <a href="http://www.flaticon.com/authors/egor-rumyantsev" title="Egor Rumyantsev">Egor Rumyantsev</a>, <a href="http://fontawesome.io/" title="Dave Gandy">Dave Gandy</a>, <a href="https://www.flaticon.com/authors/appzgear" title="Appzgear">Appzgear</a>, <div>Icons made by <a href="https://www.flaticon.com/authors/dave-gandy" title="Dave Gandy">Dave Gandy</a> from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> licensed by <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0">CC BY 3.0</a></div>')});
$(document).on("click",".ebenenknopf",function(){$(this).toggleClass("aktiv")});$(document).on("click","#ebeneOK",function(){for(var a="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(a+=String.fromCharCode(64+b));b=G(a);$("#ebenenAenderung").parent().prev(".tshareElement").attr("sb",a);$("#ebenenAenderung").wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove();k()});$(document).on("click","#ebeneAbbrechen",function(){$("#ebenenAenderung").removeAttr("id");
$(".transparent").remove()});$(document).on("click","#ebeneAlle",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv").addClass("aktiv")});$(document).on("click","#ebeneKeine",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv")});$(document).on("click","#ebeneInvertiere",function(){for(var a=0;27>a;a++)$("#bs_"+a).toggleClass("aktiv")});$(document).on("click",".ebenenAnzeige",function(){$(this).attr("id","ebenenAenderung");$("body").append("<div class='transparent'></div>");
var a=K($(this).attr("aktiv"));$(".transparent").append("<div class='fenster'>"+a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneOK'>OK</button><button id='ebeneAbbrechen'>Abbrechen</button></div>")});$(document).on("click",".editPart",function(){var a=$(this).parent().prev();if(a.hasClass("markdown")){m();var b=a.find(".originaltext").text();a.attr("id","markdown");
a.removeClass("adoccss");x=!0;z="markdown";Textabschnitt.init("#markdown","#menu",b)}else if(a.hasClass("zeichenflaeche")){m();b=a.html();a.removeClass("zeichenflaecheKlick");x=!0;z="zeichenflaeche";var c=parseInt(a.find("svg").attr("width")),d=parseInt(a.find("svg").attr("height"));QreatorBezier.init(a,c,d,"#menu",!0);QreatorBezier.loadSVG(b)}});$(document).on("click","#speichereDatei",function(){var a=$("#dateiname").val();var b=$("#checkEditor").prop("checked");var c=$("#checkPath").prop("checked"),
d=$("#pathText").val(),t="";if(null!=a)if(""==a&&(a="Notiz"),1==c&&(t=t+("<link rel='stylesheet' href='"+d+"/css/style.css'>\n")+("<link rel='stylesheet' href='"+d+"/css/font-awesome.css'>\n")+("<link rel='stylesheet' href='"+d+"/css/katex.css'>\n")+("<link rel='stylesheet' href='"+d+"/css/asciidoctor.menz.css'>\n")+("<link rel='stylesheet' href='"+d+"/css/prism.css'>\n")+("<script src='"+d+"/js/jquery.min.js'>\x3c/script>")),y=a,d=J(),$(".transparent").remove(),1==b)b=new Blob(["<!DOCTYPE html>"+
$("html").html()+"</html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_Editor_"+d+".html");else{var e="",f=0,k=0,h=0,l=0,n=!1;$(".tshareElement").each(function(){h++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&l++})});0<h&&0==l&&(n=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'";$(this).next().find(".ebenenAnzeige").attr("aktiv").includes("@");var c=" ft='"+$(this).next().find(".folientyp :selected").text()+
"' ",c=c+" style='display:none;'";$(this).next().find(".markieren").each(function(){a=$(this).hasClass("aktiv")});1==n&&(a=!0);1==a&&($(this).hasClass("zeichenflaeche")?e+="<div "+b+c+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n":$(this).hasClass("markdown")?(e+="<div "+b+c+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",f++):$(this).hasClass("datei")?(k++,e+="<div "+b+c+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):
$(this).hasClass("toc")?(e+="<div "+b+c+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n",f++):$(this).hasClass("qrcode")&&(e+="<div "+b+c+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n",f++))});var q="",m="";0<f?(q=u(n),m=$("#asciidoctorstyle").text()+$("#prism").text()+$("#fontawesome").text()):0<k&&(m=$("#asciidoctorstyle").text()+$("#prism").text()+$("#fontawesome").text());b=null;0==c?(b=new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>\n"+
window.atob("")+
"<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n<script>"+$("#jqueryminified").text()+"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script><style>"+q+m+"</style></head><body>"+e+"</body></html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_"+d+".html")):(b=new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>\n"+t+"\n"+window.atob("")+
"<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n</head><body>"+e+"</body></html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_Pfad_"+d+".html"))}});$(document).on("click","#zeigeSlideshow",function(){L(!1)});$(document).on("click","#speichereSlideshow",function(){L(!0)});$("#druckansicht").click(function(){m();var a="",b=0,c=0,d=0,e=0,f=!1;$(".tshareElement").each(function(){d++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&e++})});0<d&&0==e&&(f=!0);$(".tshareElement").each(function(){var g=!1,
d=!1,p="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'";$(this).next().find(".ebenenAnzeige").attr("aktiv").includes("@")&&(d=!0);var e=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ",e=0==d?e+" style='display:none;'":e+" style='display:true;'";$(this).next().find(".markieren").each(function(){g=$(this).hasClass("aktiv")});1==f&&(g=!0);1==g&&($(this).hasClass("zeichenflaeche")?a+="<div "+p+e+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+
"</div>\n":$(this).hasClass("markdown")?(a+="<div "+p+e+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",b++):$(this).hasClass("datei")?(c++,a+="<div "+p+e+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):$(this).hasClass("toc")?(a+="<div "+p+e+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n",b++):$(this).hasClass("qrcode")&&(a+="<div "+p+e+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n",b++))});var k="",h="";0<b?(k=u(f),h=$("#asciidoctorstyle").text()+
$("#prism").text()+$("#fontawesome").text()):0<c&&(h=$("#asciidoctorstyle").text()+$("#prism").text()+$("#fontawesome").text());var l=window.open();l.document.open();l.document.write("<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\x3c/script><script>\n"+window.atob("")+
"\x3c/script><script>"+$("#jqueryminified").text()+"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script><style>"+k+h+"</style></head><body>"+a+"<script>location.hash='tags_@';\x3c/script></body></html>")});$(document).on("click",".menu-position",function(){var a=$(this).attr("position");$(".transparent").remove();switch(a){case "left":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuLeft");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveLeft");
$("[position='left']").hide();$("[position='right']").show();$("[position='top']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "right":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#header").addClass("menuRight");$("[position='right']").hide();$("[position='left']").show();
$("[position='top']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "bottom":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveUp");$("[position='bottom']").hide();$("[position='top']").show();$("[position='right']").show();
$("[position='left']").show();$(".auswahl").removeClass("selectMenu");break;default:A()}});$(document).on("click","#recalculate",function(){m();nerdamer.flush();nerdamer.clearVars();var a=[];$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>Bitte warten ...</div>");var b=0,c=0,d=!1;$(".tshareElement").each(function(){b++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&c++})});0<b&&0==c&&(d=!0);$(".markdownText").each(function(){var b=
$(this).children(":first").next().text();if(1==d||$(this).parent(".tshareElement").next().find(".markieren").hasClass("aktiv")){$(this).children(":first").attr("id","recalculationDiv");b=Textabschnitt.recalculate(b,"#recalculationDiv");if(null!=b&&0<b.length)for(var c=0;c<b.length;c++)a.push(b[c]);$(this).children(":first").removeAttr("id")}});for(var e="\n== Inhaltsverzeichnis\n\n",f=0;f<a.length;f++){for(var e=e+"[none]\n",h=0;h<a[f][0]-1;h++)e+="*";e+=" <<"+a[f][1]+","+a[f][2]+">>\n"}e+="\n\n---";
f=Opal.hash2(["header_footer","attributes"],{header_footer:!1,attributes:["icons=font"]});e=Opal.Asciidoctor.$convert(e,f);$(".toc").each(function(){$(this).hasClass("versteckt")||($(this).next().remove(),$(this).remove())});$("#firstMenu").after("<div class='tshareElement toc adoccss'><div><div class='adoccss'>"+e+"</div></div></div>"+D);$(".transparent").remove();k()});$(document).on("click",".neueZeichenflaeche",function(){m();$(this).parent().after("<div class='zeichenflaeche' id='zeichnung'></div>"+
D);x=!0;z="zeichenflaeche";n=998;q=701;QreatorBezier.init("#zeichnung",998,701,"#menu",!0);k()});$(document).on("click",".markieren",function(){m();var a=(new Date).getTime(),b=$(this).attr("lastClick");$(this).attr("lastClick",a);if(""==b||null==b)b=""+a;b=parseInt(b);if(1E3>a-b&&0==C)$(this).removeClass("aktiv"),$(this).addClass("bereich"),C=!0;else if($(this).hasClass("bereich")&&1==C)C=!1,$(this).removeClass("bereich"),$(this).removeClass("aktiv");else if(1!=C||$(this).hasClass("bereich"))$(this).toggleClass("aktiv");
else{var c=-1,d=-1;$(this).addClass("endeBereich");$(this);var e=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("endeBereich")?(d=e,$(this).removeClass("endeBereich")):$(this).hasClass("bereich")&&(c=e);e++});e=0;c<d?$(".markieren").not(".versteckt").each(function(){e>=c&&e<=d&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),$(this).addClass("aktiv"),C=!1);e++}):c>d&&$(".markieren").not(".versteckt").each(function(){e>=d&&e<=c&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),
C=!1);e++})}k()});$("#markiereAlle").click(function(){m();C=!1;var a=0;$(".tshareElement").not(".versteckt").each(function(){a++});var b=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("aktiv")&&b++});var c=(new Date).getTime();b<a?($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").addClass("aktiv").removeClass("bereich");$(this).attr("lastClick",c)}),k()):($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").removeClass("bereich")}),
$(".wahl").each(function(){$(this).removeClass("komplett")}))});$("#auswahlLoeschen").click(function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlten Abschnitte inklusive aller Inhalte entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&(m(),$(".markieren").each(function(){$(this).hasClass("aktiv")&&($(this).parent().prev().remove(),$(this).parent().remove())}));k()});$(document).on("click",".einfuegenClipboard",function(){$(this).parent().after("<div id='einsetzen'></div>");
$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><p>Bitte f\u00fcgen Sie den Inhalt des Clipboards in das Textfeld ein (z. B. per Strg+V) und dr\u00fccken Sie dann den Knopf \"Einf\u00fcgen\"</p><textarea id='clipboardExtern' required='required' ></textArea><button id='clipboardExternEinfuegen'>Einf\u00fcgen in Dokument</button><button id='abbrechenEinsetzen'>Abbrechen</button></div>");$("#clipboardExtern").focus()});$(document).on("click","#abbrechenEinsetzen",
function(){$(".transparent").remove();$("einsetzen").remove()});$(document).on("click","#clipboardExternEinfuegen",function(){var a=$("#clipboardExtern").val();a.startsWith("JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F")?1==confirm("Sind Sie sicher, dass dieser Abschnitt eingef\u00fcgt werden soll? Dieser Schritt kann nicht mehr r\u00fcckg\u00e4ngig gemacht werden!")&&(m(),$("#einsetzen").after(a.substring(30,a.length)),$(".transparent").remove(),$("#einsetzen").remove(),N(),k()):window.alert("Der Inhalt hat das falsche Format. Es k\u00f6nnen nur Inhalte aus Tshare eingef\u00fcgt werden.")});
$(document).on("click",".einfuegen",function(){m();$(this).parent().after(M);N();k()});$(document).on("click",".markdownEinfuegen",function(){m();$(this).parent().after("<div class='markdown tshareElement' id='markdown'></div>"+Q);x=!0;z="markdown";Textabschnitt.init("#markdown","#menu");k()});$(document).on("click",".changeSeparator",function(){m();var a=0,b=0,b=parseInt($(this).parent().prev(".tshareElement").attr("status"));if(void 0==b||isNaN(b))b=0;a=(b+1)%3;$(this).parent().prev(".datei").find("hr").each(function(){$(this).attr("class",
"");$(this).addClass("style_"+a)});$(this).parent().prev(".tshareElement").removeAttr("status");$(this).parent().prev(".tshareElement").attr("status",a);$(this).removeClass("statusFarbeKnopf_"+b);$(this).addClass("statusFarbeKnopf_"+a)});$(document).on("click",".bildEinfuegen",function(){m();$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='bildInput' type='file' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");
$("#bildInput").trigger("click")});this.saveObject=function(){for(var a=window.event.srcElement,b=a.previousSibling.getAttribute("data"),c=a.previousSibling.getAttribute("type"),a=a.previousSibling.getAttribute("name"),d=b.indexOf(","),b=atob(b.substring(d+1)),d=[],e=0;e<b.length;e+=512){for(var f=b.slice(e,e+512),h=Array(f.length),k=0;k<f.length;k++)h[k]=f.charCodeAt(k);f=new Uint8Array(h);d.push(f)}c=new Blob(d,{type:c});saveAs(c,a)};$(document).on("change","#bildInput",function(){var a=$("#bildInput")[0].files[0],
b=new FileReader;b.onload=function(b){$("#bildInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");var c=new Date,c=c.getDate()+"."+(c.getMonth()+1)+"."+c.getFullYear()+", "+c.getHours()+":"+("0"+c.getMinutes()).substr(-2)+":"+("0"+c.getSeconds()).substr(-2),c="|===\n.3+|icon:file-o[5x]|Dateiname|"+a.name.split(" ").join("_")+"\n|Gr\u00f6\u00dfe in Bytes|"+a.size+"|Einf\u00fcgezeitpunkt|"+c+"\n|===",d=Opal.hash2(["header_footer","attributes"],{header_footer:!1,
attributes:["icons=font"]}),c=Opal.Asciidoctor.$convert(c,d);$("#einsetzen").before("<div class='datei tshareElement'><div style='background-color: #f8f8f7'><object style='display:none' id='dataNeu' data='' name="+a.name.split(" ").join("_")+" type='"+a.type+"'></object><button onclick='saveObject()'>"+a.name.split(" ").join("_")+" herunterladen</button><div class='adoccss'>"+c+"</div></div></div>"+D);$("#dataNeu").attr("data",b.target.result);$("#dataNeu").removeAttr("id");$(".transparent").remove();
$("#einsetzen").remove()};b.readAsDataURL(a);k()});$(document).on("click",".downloadFile",function(){var a=$(this).attr("data"),a=Base64Binary.decodeArrayBuffer(a);new Blob([a],{type:$(this).attr("datatype")});saveAs([a],$(this).attr("filename"))});$(document).on("click",".pdf",function(){m();$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='pdfInput' type='file' accept='application/pdf' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");
$("#pdfInput").trigger("click")});$(document).on("change","#pdfInput",function(){var a=$("#pdfInput")[0].files[0],b=new FileReader;b.onload=function(b){if(a.name.endsWith(".pdf")){var c=function(a){that.seitenZaehler++;var b=a.getViewport(1),b=a.getViewport(2*n/b.width),p=document.createElement("canvas");p.style.display="block";var h=p.getContext("2d");p.height=b.height;p.width=b.width;a=a.render({canvasContext:h,viewport:b});var k=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){k.call(this,
a);this.seitennummer=this.pageNumber;var b=this;a=h.getImageData(0,0,p.width,p.height);for(var c=0,d=a.data.length;c<d;c+=4){var f=a.data[c+1],m=a.data[c+2];255==a.data[c]&&255==f&&255==m&&(a.data[c+3]=0)}h.putImageData(a,0,0);r=new Image;r.onload=function(){e=p.width;l=p.height;that.zaehler++;QreatorBezier.insertImgAsSVG(this,n,q,"einsetzen"+b.seitennummer,D);console.log("Z\u00e4hler "+that.zaehler+" Seitennummer: "+b.seitennummer);that.zaehler==g&&($(".einsetzen").remove(),$(".transparent").remove())};
r.src=p.toDataURL()};d++;null!==f&&d<=g&&f.getPage(d).then(c)};$("#pdfInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");PDFJS.disableWorker=!0;var d=1,g=0,f=null;this.bildLadeZaehler=this.seitenZaehler=this.zaehler=0;that=this;PDFJS.getDocument(b.target.result).then(function(a){f=a;g=a.numPages;for(var b=0;b<g;b++)$("#einsetzen").before("<div class='einsetzen' id='einsetzen"+(b+1)+"'></div>");$("#einsetzen").remove();a.getPage(1).then(c)})}};b.readAsDataURL(a);
k()});$(document).on("click",".zeichenflaecheKlick",function(){m();var a=$(this).html();$(this).removeClass("zeichenflaecheKlick");x=!0;z="zeichenflaeche";var b=parseInt($(this).find("svg").attr("width")),c=parseInt($(this).find("svg").attr("height"));QreatorBezier.init(this,b,c,"#menu",!0);QreatorBezier.loadSVG(a)});$(document).on("click",".markdownText",function(){m();var a=$(this).find(".originaltext").text();$(this).parent().attr("id","markdown");$(this).parent().removeClass("adoccss");x=!0;z=
"markdown";Textabschnitt.init("#markdown","#menu",a)});$(document).on("change","#fileInput",function(){var a=$("#fileInput")[0].files[0],b=new FileReader;b.onload=function(a){a=a.target.result;O=[1,0,1,1,1,1,1,1,1,1];x=!1;r=y=z="";C=!1;var b=document.createElement("div");b.id="container";b.innerHTML=a;$("#content").html("<span id='firstMenu'><button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button><button class='unten imageQR'></button></span> </div>");
$(b).find(".tshareElement").each(function(){var a=$(this).attr("sb");if(null==a||""==a)a="@";var b=$(this).attr("ft");if(null==b||""==b)b="=";$(this).attr("tags");var c=" sb='"+a+"' ft='"+b+"' ";$(this).hasClass("zeichenflaeche")?($("#content").append("<div class='zeichenflaeche tshareElement zeichenflaecheKlick'"+c+">"+$(this).html()+"</div>"),$("#content").append(D)):$(this).hasClass("markdown")?($("#content").append("<div class='markdown tshareElement adoccss'"+c+">"+$(this).html()+"</div>"),$("#content").append(Q)):
$(this).hasClass("datei")?($("#content").append("<div class='datei tshareElement'"+c+">"+$(this).html()+"</div>"),$("#content").append(D)):$(this).hasClass("toc")?($("#content").append("<div class='toc tshareElement'"+c+">"+$(this).html()+"</div>"),$("#content").append(D)):$(this).hasClass("qrcode")&&($("#content").append("<div class='qrcode tshareElement'"+c+">"+$(this).html()+"</div>"),$("#content").append(D));$(".ebenenAnzeige").last().attr("aktiv",a);$(".folientyp").last().val(b);a=G(a);$(".ebenenAnzeige").last().wrap("<span id='ersetzung'></span>");
$("#ersetzung").html(a);$("#ersetzung .ebenenAnzeige").unwrap()});$(".transparent").remove();k()};b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsText(a)});$(document).on("click","#laden",function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='fileInput' type='file' accept='text/html' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");
$("#fileInput").trigger("click")});$(document).on("click","#bild",function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='imageInput' type='file' accept='image/*' required='required' ></input><button id='abbrechenBild'>Abbrechen</button><div id='parameter'></div></div>");$("#imageInput").trigger("click")});$(document).on("click","#abbrechen",function(){$(".transparent").remove();$("#einsetzen").remove()});$(document).on("click","#abbrechenBild",
function(){QreatorBezier.loescheBild();$(".transparent").remove()});$(document).on("mousemove",".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*e/n*b/100));isNaN(b)&&(b=parseInt(100*l/q*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());$(".fenster #breite").html(""+a);$(".fenster #hoehe").html(""+b);$(".fenster #xpos").html(""+c);$(".fenster #ypos").html(""+d)});$(document).on("change",
".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*e/n*b/100));isNaN(b)&&(b=parseInt(100*l/q*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(r,parseInt(c*n/100),parseInt(d*q/100),parseInt(a*n/100),parseInt(b*q/100),!1)});$(document).on("click","#bildEinfuegen",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&
(a=100*e/n*b/100);isNaN(b)&&(b=100*l/q*a/100);var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(r,parseInt(c*n/100),parseInt(d*q/100),parseInt(a*n/100),parseInt(b*q/100),!0);$(".transparent").remove()});$(document).on("click",".mittig",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*e/n*b/100));isNaN(b)&&(b=parseInt(100*l/q*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());
switch($(this).attr("typ")){case "xmitte":c=parseInt(50-a/2);break;case "ymitte":d=parseInt(50-b/2);break;case "links":c=0;break;case "rechts":c=100-a;break;case "oben":d=0;break;case "unten":d=100-b}$(".fenster #xpos").html(""+c);$("[typ='x']").val(c);$(".fenster #ypos").html(""+d);$("[typ='y']").val(d);QreatorBezier.ladeBild(r,parseInt(c*n/100),parseInt(d*q/100),parseInt(a*n/100),parseInt(b*q/100),!1)});$(document).on("change","#imageInput",function(){var a=$("#imageInput")[0].files[0],b=new FileReader;
b.onload=function(b){a.name.endsWith(".pdf")?(console.log("PDF"),PDFJS.disableWorker=!0,console.log(b.target.result),PDFJS.getDocument(b.target.result).then(function(a){a.getPage(1).then(function(a){var b=a.getViewport(2.5),c=document.createElement("canvas"),d=c.getContext("2d");c.height=b.height;c.width=b.width;a=a.render({canvasContext:d,viewport:b});var f=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){f.call(this,a);a=d.getImageData(0,0,c.width,c.height);for(var b=0,
g=a.data.length;b<g;b+=4){var h=a.data[b+1],k=a.data[b+2];255==a.data[b]&&255==h&&255==k&&(a.data[b+3]=0)}d.putImageData(a,0,0);r=new Image;r.onload=function(){e=c.width;l=c.height;w=!0;this.width>this.height&&(w=!1);1==w?(l=Math.min(q,this.height),l<q&&(l=q),e=this.width*l/this.height,e>n&&(l=l*n/e,e=n,w=!1)):(e=Math.min(n,this.width),e<n&&(e=n),l=this.height*e/this.width,l>q&&(e=e*q/l,l=q,w=!0));console.log(r.src);0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+
parseInt(100*e/n)+"'></input><span id='breite'>"+parseInt(100*e/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+parseInt(100*l/q)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*l/q)+"'></input><span id='hoehe'>"+parseInt(100*l/q)+"</span>%, Breite: <span id='breite'>"+parseInt(100*e/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,e,l,!1)};r.src=c.toDataURL()}})})):
(r=new Image,r.onload=function(){l=e=0;w=!0;this.width>this.height&&(w=!1);1==w?(l=Math.min(q,this.height),l<q&&(l=q),e=this.width*l/this.height,e>n&&(l=l*n/e,e=n,w=!1)):(e=Math.min(n,this.width),e<n&&(e=n),l=this.height*e/this.width,l>q&&(e=e*q/l,l=q,w=!0));0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*e/n)+"'></input><span id='breite'>"+parseInt(100*e/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+
parseInt(100*l/q)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*l/q)+"'></input><span id='hoehe'>"+parseInt(100*l/q)+"</span>%, Breite: <span id='breite'>"+parseInt(100*e/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,e,l,!1)},r.src=b.target.result)};
b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsDataURL(a)});$(document).on("click",".loeschen",function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlte Zeichenfl\u00e4che inklusive Inhalt entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&(m(),$(this).parent().prev().remove(),$(this).parent().remove(),k())})});
