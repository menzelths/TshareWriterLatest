var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.findInternal=function(a,c,b){a instanceof String&&(a=String(a));for(var d=a.length,l=0;l<d;l++){var q=a[l];if(c.call(b,q,l,a))return{i:l,v:q}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(a,c,b){a!=Array.prototype&&a!=Object.prototype&&(a[c]=b.value)};
$jscomp.getGlobal=function(a){return"undefined"!=typeof window&&window===a?a:"undefined"!=typeof global&&null!=global?global:a};$jscomp.global=$jscomp.getGlobal(this);$jscomp.polyfill=function(a,c,b,d){if(c){b=$jscomp.global;a=a.split(".");for(d=0;d<a.length-1;d++){var l=a[d];l in b||(b[l]={});b=b[l]}a=a[a.length-1];d=b[a];c=c(d);c!=d&&null!=c&&$jscomp.defineProperty(b,a,{configurable:!0,writable:!0,value:c})}};
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,b){return $jscomp.findInternal(this,a,b).v}},"es6-impl","es3");$jscomp.polyfill("Object.is",function(a){return a?a:function(a,b){return a===b?0!==a||1/a===1/b:a!==a&&b!==b}},"es6-impl","es3");$jscomp.polyfill("Array.prototype.includes",function(a){return a?a:function(a,b){var c=this;c instanceof String&&(c=String(c));var l=c.length;for(b=b||0;b<l;b++)if(c[b]==a||Object.is(c[b],a))return!0;return!1}},"es7","es3");
$jscomp.checkStringArgs=function(a,c,b){if(null==a)throw new TypeError("The 'this' value for String.prototype."+b+" must not be null or undefined");if(c instanceof RegExp)throw new TypeError("First argument to String.prototype."+b+" must not be a regular expression");return a+""};$jscomp.polyfill("String.prototype.includes",function(a){return a?a:function(a,b){return-1!==$jscomp.checkStringArgs(this,a,"includes").indexOf(a,b||0)}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var l=c.length,q=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var m=0;m<q&&b<l;)if(c[b++]!=a[m++])return!1;return m>=q}},"es6-impl","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};$jscomp.symbolCounter_=0;
$jscomp.Symbol=function(a){return $jscomp.SYMBOL_PREFIX+(a||"")+$jscomp.symbolCounter_++};$jscomp.initSymbolIterator=function(){$jscomp.initSymbol();var a=$jscomp.global.Symbol.iterator;a||(a=$jscomp.global.Symbol.iterator=$jscomp.global.Symbol("iterator"));"function"!=typeof Array.prototype[a]&&$jscomp.defineProperty(Array.prototype,a,{configurable:!0,writable:!0,value:function(){return $jscomp.arrayIterator(this)}});$jscomp.initSymbolIterator=function(){}};
$jscomp.arrayIterator=function(a){var c=0;return $jscomp.iteratorPrototype(function(){return c<a.length?{done:!1,value:a[c++]}:{done:!0}})};$jscomp.iteratorPrototype=function(a){$jscomp.initSymbolIterator();a={next:a};a[$jscomp.global.Symbol.iterator]=function(){return this};return a};$jscomp.makeIterator=function(a){$jscomp.initSymbolIterator();var c=a[Symbol.iterator];return c?c.call(a):$jscomp.arrayIterator(a)};$jscomp.owns=function(a,c){return Object.prototype.hasOwnProperty.call(a,c)};
$jscomp.polyfill("WeakMap",function(a){function c(a){$jscomp.owns(a,d)||$jscomp.defineProperty(a,d,{value:{}})}function b(a){var b=Object[a];b&&(Object[a]=function(a){c(a);return b(a)})}if(function(){if(!a||!Object.seal)return!1;try{var b=Object.seal({}),B=Object.seal({}),c=new a([[b,2],[B,3]]);if(2!=c.get(b)||3!=c.get(B))return!1;c.delete(b);c.set(B,4);return!c.has(b)&&4==c.get(B)}catch(G){return!1}}())return a;var d="$jscomp_hidden_"+Math.random().toString().substring(2);b("freeze");b("preventExtensions");
b("seal");var l=0,q=function(a){this.id_=(l+=Math.random()+1).toString();if(a){$jscomp.initSymbol();$jscomp.initSymbolIterator();a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};q.prototype.set=function(a,b){c(a);if(!$jscomp.owns(a,d))throw Error("WeakMap key fail: "+a);a[d][this.id_]=b;return this};q.prototype.get=function(a){return $jscomp.owns(a,d)?a[d][this.id_]:void 0};q.prototype.has=function(a){return $jscomp.owns(a,d)&&$jscomp.owns(a[d],this.id_)};q.prototype.delete=
function(a){return $jscomp.owns(a,d)&&$jscomp.owns(a[d],this.id_)?delete a[d][this.id_]:!1};return q},"es6-impl","es3");$jscomp.MapEntry=function(){};
$jscomp.polyfill("Map",function(a){if(!$jscomp.ASSUME_NO_NATIVE_MAP&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([[b,"s"]]));if("s"!=c.get(b)||1!=c.size||c.get({x:4})||c.set({x:4},"t")!=c||2!=c.size)return!1;var d=c.entries(),l=d.next();if(l.done||l.value[0]!=b||"s"!=l.value[1])return!1;l=d.next();return l.done||4!=l.value[0].x||"t"!=l.value[1]||!d.next().done?!1:!0}catch(J){return!1}}())return a;$jscomp.initSymbol();
$jscomp.initSymbolIterator();var c=new WeakMap,b=function(a){this.data_={};this.head_=q();this.size=0;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};b.prototype.set=function(a,b){var c=d(this,a);c.list||(c.list=this.data_[c.id]=[]);c.entry?c.entry.value=b:(c.entry={next:this.head_,previous:this.head_.previous,head:this.head_,key:a,value:b},c.list.push(c.entry),this.head_.previous.next=c.entry,this.head_.previous=c.entry,this.size++);return this};b.prototype.delete=
function(a){a=d(this,a);return a.entry&&a.list?(a.list.splice(a.index,1),a.list.length||delete this.data_[a.id],a.entry.previous.next=a.entry.next,a.entry.next.previous=a.entry.previous,a.entry.head=null,this.size--,!0):!1};b.prototype.clear=function(){this.data_={};this.head_=this.head_.previous=q();this.size=0};b.prototype.has=function(a){return!!d(this,a).entry};b.prototype.get=function(a){return(a=d(this,a).entry)&&a.value};b.prototype.entries=function(){return l(this,function(a){return[a.key,
a.value]})};b.prototype.keys=function(){return l(this,function(a){return a.key})};b.prototype.values=function(){return l(this,function(a){return a.value})};b.prototype.forEach=function(a,b){for(var c=this.entries(),d;!(d=c.next()).done;)d=d.value,a.call(b,d[1],d[0],this)};b.prototype[Symbol.iterator]=b.prototype.entries;var d=function(a,b){var d=b&&typeof b;"object"==d||"function"==d?c.has(b)?d=c.get(b):(d=""+ ++m,c.set(b,d)):d="p_"+b;var l=a.data_[d];if(l&&$jscomp.owns(a.data_,d))for(a=0;a<l.length;a++){var q=
l[a];if(b!==b&&q.key!==q.key||b===q.key)return{id:d,list:l,index:a,entry:q}}return{id:d,list:l,index:-1,entry:void 0}},l=function(a,b){var c=a.head_;return $jscomp.iteratorPrototype(function(){if(c){for(;c.head!=a.head_;)c=c.previous;for(;c.next!=c.head;)return c=c.next,{done:!1,value:b(c)};c=null}return{done:!0,value:void 0}})},q=function(){var a={};return a.previous=a.next=a.head=a},m=0;return b},"es6-impl","es3");
$jscomp.polyfill("Set",function(a){if(!$jscomp.ASSUME_NO_NATIVE_SET&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([b]));if(!c.has(b)||1!=c.size||c.add(b)!=c||1!=c.size||c.add({x:4})!=c||2!=c.size)return!1;var l=c.entries(),q=l.next();if(q.done||q.value[0]!=b||q.value[1]!=b)return!1;q=l.next();return q.done||q.value[0]==b||4!=q.value[0].x||q.value[1]!=q.value[0]?!1:l.next().done}catch(m){return!1}}())return a;
$jscomp.initSymbol();$jscomp.initSymbolIterator();var c=function(a){this.map_=new Map;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)this.add(b.value)}this.size=this.map_.size};c.prototype.add=function(a){this.map_.set(a,a);this.size=this.map_.size;return this};c.prototype.delete=function(a){a=this.map_.delete(a);this.size=this.map_.size;return a};c.prototype.clear=function(){this.map_.clear();this.size=0};c.prototype.has=function(a){return this.map_.has(a)};c.prototype.entries=function(){return this.map_.entries()};
c.prototype.values=function(){return this.map_.values()};c.prototype.keys=c.prototype.values;c.prototype[Symbol.iterator]=c.prototype.values;c.prototype.forEach=function(a,c){var b=this;this.map_.forEach(function(d){return a.call(c,d,d,b)})};return c},"es6-impl","es3");
$jscomp.polyfill("Array.from",function(a){return a?a:function(a,b,d){$jscomp.initSymbolIterator();b=null!=b?b:function(a){return a};var c=[],q=a[Symbol.iterator];if("function"==typeof q)for(a=q.call(a);!(q=a.next()).done;)c.push(b.call(d,q.value));else for(var q=a.length,m=0;m<q;m++)c.push(b.call(d,a[m]));return c}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.endsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"endsWith");a+="";void 0===b&&(b=c.length);b=Math.max(0,Math.min(b|0,c.length));for(var l=a.length;0<l&&0<b;)if(c[--b]!=a[--l])return!1;return 0>=l}},"es6-impl","es3");
var saveAs=saveAs||function(a){if("undefined"===typeof navigator||!/MSIE [1-9]\./.test(navigator.userAgent)){var c=a.document.createElementNS("http://www.w3.org/1999/xhtml","a"),b="download"in c,d=/Version\/[\d\.]+.*Safari/.test(navigator.userAgent),l=a.webkitRequestFileSystem,q=a.requestFileSystem||l||a.mozRequestFileSystem,m=function(b){(a.setImmediate||a.setTimeout)(function(){throw b;},0)},B=0,I=function(b){var c=function(){"string"===typeof b?(a.URL||a.webkitURL||a).revokeObjectURL(b):b.remove()};
a.chrome?c():setTimeout(c,500)},G=function(a,b,c){b=[].concat(b);for(var d=b.length;d--;){var l=a["on"+b[d]];if("function"===typeof l)try{l.call(a,c||a)}catch(k){m(k)}}},H=function(a){return/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(a.type)?new Blob(["\ufeff",a],{type:a.type}):a},J=function(m,x,r){r||(m=H(m));var v=this;r=m.type;var n=!1,k=function(){G(v,["writestart","progress","write","writeend"])},z=function(){if(w&&d&&"undefined"!==typeof FileReader){var b=
new FileReader;b.onloadend=function(){var a=b.result;w.location.href="data:attachment/file"+a.slice(a.search(/[,;]/));v.readyState=v.DONE;k()};b.readAsDataURL(m);v.readyState=v.INIT}else{if(n||!g)g=(a.URL||a.webkitURL||a).createObjectURL(m);w?w.location.href=g:void 0==a.open(g,"_blank")&&d&&(a.location.href=g);v.readyState=v.DONE;k();I(g)}},A=function(a){return function(){if(v.readyState!==v.DONE)return a.apply(this,arguments)}},y={create:!0,exclusive:!1};v.readyState=v.INIT;x||(x="download");if(b){var g=
(a.URL||a.webkitURL||a).createObjectURL(m);setTimeout(function(){c.href=g;c.download=x;var a=new MouseEvent("click");c.dispatchEvent(a);k();I(g);v.readyState=v.DONE})}else{if(a.chrome&&r&&"application/octet-stream"!==r){var h=m.slice||m.webkitSlice;m=h.call(m,0,m.size,"application/octet-stream");n=!0}l&&"download"!==x&&(x+=".download");if("application/octet-stream"===r||l)var w=a;q?(B+=m.size,q(a.TEMPORARY,B,A(function(a){a.root.getDirectory("saved",y,A(function(a){var b=function(){a.getFile(x,y,
A(function(a){a.createWriter(A(function(b){b.onwriteend=function(b){w.location.href=a.toURL();v.readyState=v.DONE;G(v,"writeend",b);I(a)};b.onerror=function(){var a=b.error;a.code!==a.ABORT_ERR&&z()};["writestart","progress","write","abort"].forEach(function(a){b["on"+a]=v["on"+a]});b.write(m);v.abort=function(){b.abort();v.readyState=v.DONE};v.readyState=v.WRITING}),z)}),z)};a.getFile(x,{create:!1},A(function(a){a.remove();b()}),A(function(a){a.code===a.NOT_FOUND_ERR?b():z()}))}),z)}),z)):z()}},
x=J.prototype;if("undefined"!==typeof navigator&&navigator.msSaveOrOpenBlob)return function(a,b,c){c||(a=H(a));return navigator.msSaveOrOpenBlob(a,b||"download")};x.abort=function(){this.readyState=this.DONE;G(this,"abort")};x.readyState=x.INIT=0;x.WRITING=1;x.DONE=2;x.error=x.onwritestart=x.onprogress=x.onwrite=x.onabort=x.onerror=x.onwriteend=null;return function(a,b,c){return new J(a,b,c)}}}("undefined"!==typeof self&&self||"undefined"!==typeof window&&window||this.content);
"undefined"!==typeof module&&module.exports?module.exports.saveAs=saveAs:"undefined"!==typeof define&&null!==define&&null!=define.amd&&define([],function(){return saveAs});
$(function(){function a(a){for(var e="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(e+=String.fromCharCode(64+b));$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".ebenenAnzeige").each(function(){var p=$(this).attr("aktiv");if("plus"==a)for(var b=0;b<e.length;b++)p.includes(e.charAt(b))||(p+=e.charAt(b));else if("minus"==a){for(var N="",b=0;b<p.length;b++)e.includes(p.charAt(b))||(N+=p.charAt(b));p=N}else"gleich"==a&&(p=e);$(this).attr("aktiv",p);b=H(p);$(this).parent().prev(".tshareElement").attr("sb",
p);$(this).wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove()})})}function c(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv").addClass("aktiv")})}function b(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+
parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv")})}function d(){for(var a=Array(27),p=Array(27),b=0;27>b;b++)a[b]=0,p[b]=0;$(".ebenenAnzeige").each(function(){if(!$(this).parent().hasClass("versteckt")){for(var e=$(this).attr("aktiv").toUpperCase(),b=0;b<e.length;b++)p[e.charCodeAt(b)-64]++;if($(this).parent().find(".markieren").hasClass("aktiv"))for(b=0;b<e.length;b++)a[e.charCodeAt(b)-64]++}});return[a,p]}function l(a){var e=!1;a.toUpperCase().startsWith("TAGS_")&&(a=a.split("_")[1],
""==a&&(e=!0),$(".tshareElement").each(function(){var p=$(this).attr("sb");if(null==p||""==p)p="@";for(var b=-1,c=0;c<a.length;c++)p.includes(a.charAt(c))&&(b=c,c=a.length);0<=b||1==e?($(this).attr("style","display:true;"),$(this).next().attr("style","display:true"),$(this).removeClass("versteckt"),$(this).next().removeClass("versteckt"),$(this).next().find(".markieren").removeClass("versteckt")):($(this).attr("style","display:none;"),$(this).next().attr("style","display:none"),$(this).removeClass("versteckt").addClass("versteckt"),
$(this).next().removeClass("versteckt").addClass("versteckt"),$(this).next().find(".markieren").removeClass("versteckt").addClass("versteckt"))}),m(),$("#hideShow").removeClass("komplett").addClass("komplett"))}function q(){$(".tshareElement").each(function(){var a=parseInt($(this).attr("sichtbarkeit"));0==v[a]?$(this).attr("visibility","hidden"):$(this).removeAttr("visibility")})}function m(){for(var a=d(),p=a[0],a=a[1],b=0;27>b;b++)0<a[b]&&p[b]==a[b]?($("[wahl='"+b+"']").removeClass("komplett").addClass("komplett"),
$("[wahl='"+b+"']").attr("style","display:true")):0<a[b]&&p[b]<a[b]?($("[wahl='"+b+"']").removeClass("komplett"),$("[wahl='"+b+"']").attr("style","display:true")):0==a[b]&&($("[wahl='"+b+"']").removeClass("komplett"),$("[wahl='"+b+"']").attr("style","display:none"))}function B(){$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuTop");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");
$("#content").addClass("moveDown");$("[position='top']").hide();$("[position='right']").show();$("[position='left']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu")}function I(){var a=new Date,b=a.getMonth()+1,c="";10>b&&(c="0");var c=c+b,f=a.getDate(),b="";10>f&&(b="0");var b=b+f,f="",t=a.getHours();10>t&&(f="0");var f=f+t,t="",u=a.getMinutes();10>u&&(t="0");var t=t+u,u="",d=a.getSeconds();10>d&&(u="0");u+=d;return""+(1900+a.getYear())+c+b+"_"+f+t+u}function G(a){$("#einsetzen").html("");
new QRCode($("#einsetzen")[0],{text:$("#qrLink").val(),width:parseInt($("#qrKante").val()),height:parseInt($("#qrKante").val()),colorDark:$("#qrFarbe").val(),colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H});var b="";1==$("#qrTextZeigen").prop("checked")&&(b=$("#qrText").val());1==a&&$("#einsetzen").after(E);$("#einsetzen").append("<div class='qrinfo' qrtext='"+$("#qrLink").val()+"' qrcapture='"+b+"' qrsize='"+parseInt($("#qrKante").val())+"' qrfarbe='"+$("#qrFarbe").val()+"'></div><div class='qrcenter'>"+
b+"</div>");$("#einsetzen").removeAttr("id");$(".transparent").remove();m()}function H(a){var b=document.createElement("canvas");a=a.toUpperCase();var e=b.getContext("2d");b.height=35;b.width=106;e.font="10px Sans-Serif";e.fillStyle="#ffff00";e.fillStyle="#000000";for(var f=0;27>f;f++)a.includes(String.fromCharCode(64+f))?(e.fillStyle="#ffffaa",e.fillRect(f%9*12,12*parseInt(f/9),12,12),e.fillStyle="#000000"):e.fillStyle="#aaaaaa",e.fillText(String.fromCharCode(64+f),2+f%9*12,10+12*parseInt(f/9));
e="";for(f=0;f<a.length;f++)e.includes(a.charAt(f))||(e+=a.charAt(f));return"<span class='ebenenAnzeige' aktiv='"+e+"'><img src='"+b.toDataURL()+"'   ></img></span>"}function J(a){for(var b="<div>",e=0;27>e;e++){var f="";a.includes(String.fromCharCode(64+e))&&(f="aktiv");b+="<button id='bs_"+e+"' class='ebenenknopf "+f+"'>"+String.fromCharCode(64+e)+"</button>";0==(e+1)%9&&(b+="<br>")}return b+"</div>"}function x(a){for(var b="",e=".katex .delimsizing.size1;.katex .delimsizing.size2;.katex .delimsizing.size3;.katex .delimsizing.size4;.katex .delimsizing.mult .delim-size1 > span;.katex .delimsizing.mult .delim-size4 > span;.katex .op-symbol.small-op;.katex .op-symbol.large-op;.katex .mathit;.katex .mathbf;.katex .amsrm;.katex .mathbb;.katex .mathcal;.katex .mathfrak;.katex .mathtt;.katex .mathscr;.katex .mathsf;.katex .mainit".split(";"),
b="KaTeX_Size1 KaTeX_Size2 KaTeX_Size3 KaTeX_Size4 KaTeX_Size1 KaTeX_Size4 KaTeX_Size1 KaTeX_Size2 KaTeX_Main KaTeX_Main KaTeX_Math KaTeX_Main KaTeX_AMS KaTeX_AMS KaTeX_Caligraphic KaTeX_Fraktur KaTeX_Typewriter KaTeX_Script KaTeX_SansSerif KaTeX_Main".split(" "),f=0;f<e.length;f++){var c=0;$(e[f]).each(function(){var b=!1;$(this).parents(".tshareElement").next().find(".markieren").each(function(){b=$(this).hasClass("aktiv")});1!=b&&1!=a||c++});e[f]=c}for(var u=new Set,f=0;f<e.length;f++)0<e[f]&&
u.add(b[f]);0<u.size&&(u.add("KaTeX_Main"),u.add("KaTeX_Math"));e=Array.from(u);console.log("Schriften: ");for(var u="",d=$("#katexStyle").text().split("/* KateX_Fonts END */"),b=d[0].split("/**/"),f=0;f<e.length;f++)for(var g=e[f],h=0;h<b.length;h++)-1!=b[h].indexOf(g)&&(u+=b[h].trim());return b=u+d[1]}function L(a){var b=$("#dateiname").val();null!=b&&""==b&&(b="Slideshow");var e=I(),b=b+"_"+e+".html";$(".transparent").remove();var c="",t="",u="",d=0,g=0,h=0,l=0,m=!1,k=0;$(".tshareElement").each(function(){h++;
$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&l++})});0<h&&0==l&&(m=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",e=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){a=$(this).hasClass("aktiv")});var p="=";$(this).next().find(".folientyp").each(function(){p=$(this).val()});"="==p?(u=t="",0==k&&(t="<section>",k=1)):"+"==p?(0==k?t="<section>":
1==k?t="</section><section>":2==k&&(t="</section></section><section>"),u="",k=1):"++"==p?(0==k?t="<section><section>":1==k?t="</section><section><section>":2==k&&(t="</section><section>"),k=2,u=""):"/"==p&&(t="<span class='fragment'>",u="</span>");1==m&&(a=!0);1==a&&($(this).hasClass("zeichenflaeche")?c+=t+"<div "+b+e+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n"+u:$(this).hasClass("markdown")?(c+=t+"<div "+b+e+" class='markdown tshareElement adoccss'>"+
$(this).html()+"</div>\n"+u,d++):$(this).hasClass("datei")?(g++,c+=t+"<div "+b+e+" class='datei tshareElement'>"+$(this).html()+"</div>\n"+u):$(this).hasClass("toc")?(c+=t+"<div "+b+e+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n"+u,d++):$(this).hasClass("qrcode")&&(c+=t+"<div "+b+e+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n"+u,d++))});1==k?c+="</section>":2==k&&(c+="</section></section>");var n=e="";0<d?(e=x(m),n=$("#asciidoctorstyle").text()):0<g&&(n=$("#asciidoctorstyle").text());
0==a?(b=window.open(),b.document.open(),b.document.write("<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+n+e+window.atob("")+
window.atob("")+
"</style></head><body><div class='reveal'><div class='slides'>"+c+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>")):(a=new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+n+e+window.atob("")+
window.atob("")+
"</style></head><body><div class='reveal'><div class='slides'>"+c+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>"],{type:"text/html;charset=utf-8"}),saveAs(a,b))}function O(){var a="";r();$(".markieren").each(function(b,e){if($(this).hasClass("aktiv")){b=$(this).parent().prev();var c=$(this).parent().prev().attr("sb");e=$(this).parent().outerHTML();if(null==c||""==c)c="@";$(this).parent().prev().attr("ft");c=" sb='"+c+"' ft='='";b.hasClass("markdown")?(klasse="markdown adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+
e):b.hasClass("zeichenflaeche")?(klasse="zeichenflaeche zeichenflaecheKlick",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+e):b.hasClass("datei")?(klasse="datei",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+e):b.hasClass("toc")?(klasse="toc adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+e):b.hasClass("qrcode")&&(klasse="qrcode adoccss",a+="<div class='tshareElement "+klasse+"'"+c+">"+b.html()+"</div>"+e)}});K=a;return a="JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+
a}function r(){1==bearbeitungAn&&("zeichenflaeche"==z?(QreatorBezier.showSVG("zeichenflaeche"),bearbeitungAn=!1):"markdown"==z&&(Textabschnitt.zeigeMarkdown("#markdownTable"),bearbeitungAn=!1,$("#markdown").addClass("adoccss"),$("#markdown").removeAttr("id")))}B();var v=[1,0,1,1,1,1,1,1,1,1],n=998,k=701;bearbeitungAn=!1;var z="",A="",y="",g=0,h=0,w=!1,K="",D=!1,F="";$(document).on("click",".wahl",function(){r();$(this).hasClass("komplett")?(b($(this).attr("wahl")),$(this).removeClass("komplett")):
(c($(this).attr("wahl")),$(this).removeClass("komplett"),$(this).addClass("komplett"));m()});$(document).on("click","#ebenenzuweisung",function(){$("body").append("<div class='transparent'></div>");var a=J("");$(".transparent").append("<div class='fenster'>"+a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneZuweisungPlus'>Tags hinzuf\u00fcgen</button><button id='ebeneZuweisungMinus'>Tags abziehen</button><button id='ebeneZuweisungGleich'>Tags \u00fcbernehmen</button></br><button id='ebeneAbbrechen'>Abbrechen</button></div>")});
$(document).on("click","#ebeneZuweisungPlus",function(){a("plus");m();$(".transparent").remove()});$(document).on("click","#ebeneZuweisungMinus",function(){a("minus");m();$(".transparent").remove()});$(document).on("click","#ebeneZuweisungGleich",function(){a("gleich");m();$(".transparent").remove()});for(var F="<select class='sichtbarkeit'>",C=0;27>C;C++)F=0==C?F+("<option selected sichtbarkeit='"+C+"'>"+String.fromCharCode(64+C)+"</option> "):F+("<option sichtbarkeit='"+C+"'>"+String.fromCharCode(64+
C)+"</option> ");F+="</select>";$(document).on("change",".folientyp",function(a){a.preventDefault();a=$(this).val();$(this).parent().prev(".tshareElement").attr("ft",a)});$(document).on("change",".auswahlebene",function(a){a.preventDefault();var b=parseInt($(this).prop("selectedIndex"));0<b&&$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".sichtbarkeit").val(""+String.fromCharCode(64+b))});$(this).val("-");m()});$(window).on("hashchange",function(){var a=location.hash.substring(1).toUpperCase();
l(a)});$(document).on("change",".sichtbarkeit",function(a){a.preventDefault();a=parseInt($(this).prop("selectedIndex"));$(this).parent().prev().attr("sichtbarkeit",a);q();m()});var C=H("@"),E="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select>"+
C+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button><button class='unten imageQR'></button></span>",M="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select><button class='imagePen editPart oben'></button>"+
C+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button><button class='unten imageQR'></button></span>";$(document).on("click","#hideShow",function(){r();$(this).hasClass("komplett")?($(".tshareElement").each(function(){$(this).attr("style",
"display:true");$(this).removeClass("versteckt")}),$(".menuBar").each(function(){$(this).attr("style","display:true");$(this).removeClass("versteckt");$(this).find(".markieren").removeClass("versteckt")}),$(this).removeClass("komplett"),location.hash=""):($(".markieren").each(function(){$(this).hasClass("aktiv")||($(this).parent().prev(".tshareElement").attr("style","display:none"),$(this).parent().attr("style","display:none"),$(this).parent().removeClass("versteckt").addClass("versteckt"),$(this).removeClass("versteckt").addClass("versteckt"),
$(this).parent().prev(".tshareElement").removeClass("versteckt").addClass("versteckt"))}),$(this).addClass("komplett"));m()});$(document).ready(function(){jQuery.fn.outerHTML=function(a){return a?this.before(a).remove():jQuery("<p>").append(this.eq(0).clone()).html()};new Clipboard("#kopieren",{text:function(a){return O()}});new Clipboard(".kopiereAbschnitt",{text:function(a){r();var b="",e=$(a).parent().prev(),c=$(a).parent().outerHTML(),p=e.attr("sb");if(null==p||""==p)p="@";e.attr("ft");p=" sb='"+
p+"' ft='=' ";e.hasClass("markdown")?b="<div class='tshareElement markdown adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c:e.hasClass("zeichenflaeche")?b="<div class='tshareElement zeichenflaeche zeichenflaecheKlick'"+p+">"+$(a).parent().prev().html()+"</div>"+c:e.hasClass("datei")?b="<div class='tshareElement datei'"+p+">"+$(a).parent().prev().html()+"</div>"+c:e.hasClass("toc")?b="<div class='tshareElement toc adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c:e.hasClass("qrcode")&&(b=
"<div class='tshareElement qrcode adoccss'"+p+">"+$(a).parent().prev().html()+"</div>"+c);K=b;return"JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+b}});F="";for(var a=0;27>a;a++)F+="<button class='wahl allgemein knopfStandard' wahl='"+a+"' >"+String.fromCharCode(64+a)+"</button>";$("#ebenenwahl").html(F);$(".tshareElement").each(function(){var a=$(this).attr("ft");if(null==a||""==a)a="=";$(this).next().find(".folientyp").val(a)});m()});window.addEventListener("beforeunload",function(a){return a.returnValue=
"Eventuelle \u00c4nderungen am Dokument k\u00f6nnten verloren gehen!"});document.addEventListener("paste",function(a){if(1==$("#rahmen").length&&0==$(".transparent").length)if(a=a.clipboardData.items)for(var b=0;b<a.length;b++){var e=a[b];if(e.type.startsWith("image/")){var e=e.getAsFile(),c=new FileReader;c.onload=function(a){y=new Image;y.onload=function(){h=g=0;w=!0;this.width>this.height&&(w=!1);1==w?(h=Math.min(k,this.height),h<k&&(h=k),g=this.width*h/this.height,g>n&&(h=h*n/g,g=n,w=!1)):(g=
Math.min(n,this.width),g<n&&(g=n),h=this.height*g/this.width,h>k&&(g=g*k/h,h=k,w=!0));$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><div id='parameter'></div><button id='abbrechenBild'>Abbrechen</button></div>");0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*g/n)+"'></input><span id='breite'>"+parseInt(100*g/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+
parseInt(100*h/k)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*h/k)+"'></input><span id='hoehe'>"+parseInt(100*h/k)+"</span>%, Breite: <span id='breite'>"+parseInt(100*g/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(y,0,0,g,h,!1)};y.src=a.target.result};
c.onerror=function(a){console.log(a.target.error.name)};c.onprogress=function(a){console.log(a.loaded,a.total)};c.readAsDataURL(e)}}else console.log("fileList is null.")},!1);$(document).on("click",".imageQR",function(){$("body").append("<div class='transparent'></div>");$(this).parent().after("<div id='einsetzen' class='tshareElement qrcode adoccss' sb='@' ft='='></div>");$(".transparent").append("<div class='fenster'>QR-Code<p>Zu codierender Text:<br> <textarea  cols='80' rows='8' id='qrLink' ></textarea><br><button id='qrdieseSeite'>Aktuelle Seite verwenden</button><p><input type='checkbox' id='qrTextZeigen'>Bildunterschrift:</input><br><input type='text' size='80' id='qrText' ></input><br><p>Kantenl\u00e4nge in Pixel:<br><input type='text' size='15' id='qrKante' value='250'></input><p>Farbe:<br><input id='qrFarbe'  type='color' value='#000000' list='color' /><datalist id='color'><option>#000000</option><option>#ff0000</option><option>#0000ff</option><option>#ff9900</option><option>#008000</option><option>#a52a2a</option><option>#999999</option></datalist><br><p><button id='qrfertig'>OK</button><button id='qrabbrechen'>Abbrechen</button></div>")});
$(document).on("click","#qrabbrechen",function(){$("#einsetzen").remove();$(".transparent").remove()});$(document).on("click","#qrabbrechen2",function(){$("#ersetzen").removeAttr("id");$(".transparent").remove()});$(document).on("click","#qrdieseSeite",function(){var a=location.href;$("#qrLink").val(a);$("#qrText").val(a)});$(document).on("click","#qrfertig",function(){G(!0)});$(document).on("click","#qrfertig2",function(){G(!1)});$(document).on("click",".qrcode",function(){var a=$(this).find(".qrinfo").attr("qrtext"),
b=$(this).find(".qrinfo").attr("qrcapture"),c=$(this).find(".qrinfo").attr("qrsize"),f=$(this).find(".qrinfo").attr("qrfarbe");$(this).attr("id","einsetzen");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>QR-Code<p>Zu codierender Text:<br> <textarea  cols='80' rows='8' id='qrLink' ></textarea><br><button id='qrdieseSeite'>Aktuelle Seite verwenden</button><p><input type='checkbox' id='qrTextZeigen'>Bildunterschrift:</input><br><input type='text' size='80' id='qrText' ></input><br><p>Kantenl\u00e4nge in Pixel:<br><input type='text' size='15' id='qrKante' value='250'></input><p>Farbe:<br><input id='qrFarbe'  type='color' value='#000000' list='color' /><datalist id='color'><option>#000000</option><option>#ff0000</option><option>#0000ff</option><option>#ff9900</option><option>#008000</option><option>#a52a2a</option><option>#999999</option></datalist><br><p><button id='qrfertig2'>OK</button><button id='qrabbrechen2'>Abbrechen</button></div>");
$("#qrLink").val(a);$("#qrText").val(b);""!=b.trim()&&$("#qrTextZeigen").prop("checked",!0);$("#qrKante").val(c);$("#qrFarbe").val(f)});$("#speichern").click(function(){r();$("body").append("<div class='transparent'></div>");var a=A;""==A&&(a="Notiz");$(".transparent").append("<div class='fenster'>Speichern der aktuellen Datei (Zeitstempel und Endung werden angeh\u00e4ngt)<br>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><br><input type='checkbox' id='checkEditor'>Editor in Datei einbetten</input><br><input type='checkbox' id='checkPath'>Pfad zu Bibliotheken ber\u00fccksichtigen: </input><input type='text' id='pathText' value='../assets'></input><br><button id='speichereDatei'>Speichern</button><button id='abbrechen'>Abbrechen</button></div>")});
$("#slideshow").click(function(){r();$("body").append("<div class='transparent'></div>");var a=A;""==A&&(a="Slideshow");$(".transparent").append("<div class='fenster'>Speichern / Zeigen der aktuellen Slideshow (Zeitstempel und Endung werden beim Speichern angeh\u00e4ngt)<br>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><br><button id='speichereSlideshow'>Speichern</button><button id='zeigeSlideshow'>Zeigen</button><button id='abbrechen'>Abbrechen</button></div>")});$("#menuansicht").click(function(){$("body").append("<div class='transparent'></div>");
$(".transparent").append("<div class='fenster'>Position des Men\u00fcs w\u00e4hlen:<br><button class='menu-position' position='left'>Men\u00fc links</button><button class='menu-position' position='top'>Men\u00fc oben</button><button class='menu-position' position='bottom'>Men\u00fc unten</button><button class='menu-position' position='right'>Men\u00fc rechts</button><br><button id='abbrechen'>Abbrechen</button></div>");$(".fenster").append('<p>Info:<br><div>Icons made by <a href="http://www.freepik.com" title="Freepik">Freepik</a>, <a href="http://www.flaticon.com/authors/yannick" title="Yannick">Yannick</a>, <a href="http://www.flaticon.com/authors/picol" title="Picol">Picol</a>, <a href="http://www.flaticon.com/authors/situ-herrera" title="Situ Herrera">Situ Herrera</a>, <a href="https://www.flaticon.com/authors/google" title="Google">Google</a>, <a href="http://www.flaticon.com/authors/freepik">freepik</a>, <a href="http://www.flaticon.com/authors/egor-rumyantsev" title="Egor Rumyantsev">Egor Rumyantsev</a>, <a href="http://fontawesome.io/" title="Dave Gandy">Dave Gandy</a>, <a href="https://www.flaticon.com/authors/appzgear" title="Appzgear">Appzgear</a>, <div>Icons made by <a href="https://www.flaticon.com/authors/dave-gandy" title="Dave Gandy">Dave Gandy</a> from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> licensed by <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0">CC BY 3.0</a></div>')});
$(document).on("click",".ebenenknopf",function(){$(this).toggleClass("aktiv")});$(document).on("click","#ebeneOK",function(){for(var a="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(a+=String.fromCharCode(64+b));b=H(a);$("#ebenenAenderung").parent().prev(".tshareElement").attr("sb",a);$("#ebenenAenderung").wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove();m()});$(document).on("click","#ebeneAbbrechen",function(){$("#ebenenAenderung").removeAttr("id");
$(".transparent").remove()});$(document).on("click","#ebeneAlle",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv").addClass("aktiv")});$(document).on("click","#ebeneKeine",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv")});$(document).on("click","#ebeneInvertiere",function(){for(var a=0;27>a;a++)$("#bs_"+a).toggleClass("aktiv")});$(document).on("click",".ebenenAnzeige",function(){$(this).attr("id","ebenenAenderung");$("body").append("<div class='transparent'></div>");
var a=J($(this).attr("aktiv"));$(".transparent").append("<div class='fenster'>"+a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneOK'>OK</button><button id='ebeneAbbrechen'>Abbrechen</button></div>")});$(document).on("click",".editPart",function(){var a=$(this).parent().prev();if(a.hasClass("markdown")){r();var b=a.find(".originaltext").text();a.attr("id","markdown");
a.removeClass("adoccss");bearbeitungAn=!0;z="markdown";Textabschnitt.init("#markdown","#menu",b)}else if(a.hasClass("zeichenflaeche")){r();b=a.html();a.removeClass("zeichenflaecheKlick");bearbeitungAn=!0;z="zeichenflaeche";var c=parseInt(a.find("svg").attr("width")),f=parseInt(a.find("svg").attr("height"));QreatorBezier.init(a,c,f,"#menu",!0);QreatorBezier.loadSVG(b)}});$(document).on("click","#speichereDatei",function(){var a=$("#dateiname").val();var b=$("#checkEditor").prop("checked");var c=$("#checkPath").prop("checked"),
f=$("#pathText").val(),t="";if(null!=a)if(""==a&&(a="Notiz"),1==c&&(t=t+("<link rel='stylesheet' href='"+f+"/css/style.css'>\n")+("<link rel='stylesheet' href='"+f+"/css/font-awesome.css'>\n")+("<link rel='stylesheet' href='"+f+"/css/katex.css'>\n")+("<link rel='stylesheet' href='"+f+"/css/asciidoctor.menz.css'>\n")+("<link rel='stylesheet' href='"+f+"/css/prism.css'>\n")+("<script src='"+f+"/js/jquery.min.js'>\x3c/script>")),A=a,f=I(),$(".transparent").remove(),1==b)b=new Blob(["<!DOCTYPE html>"+
$("html").html()+"</html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_Editor_"+f+".html");else{var d="",g=0,h=0,k=0,m=0,l=!1;$(".tshareElement").each(function(){k++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&m++})});0<k&&0==m&&(l=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",c=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){a=
$(this).hasClass("aktiv")});1==l&&(a=!0);1==a&&($(this).hasClass("zeichenflaeche")?d+="<div "+b+c+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n":$(this).hasClass("markdown")?(d+="<div "+b+c+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",g++):$(this).hasClass("datei")?(h++,d+="<div "+b+c+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):$(this).hasClass("toc")?(d+="<div "+b+c+" class='toc tshareElement adoccss'>"+$(this).html()+
"</div>\n",g++):$(this).hasClass("qrcode")&&(d+="<div "+b+c+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n",g++))});var n="",q="";0<g?(n=x(l),q=$("#asciidoctorstyle").text()):0<h&&(q=$("#asciidoctorstyle").text());b=null;0==c?(b=new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>\n"+window.atob("")+
"<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n<script>"+$("#jqueryminified").text()+"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script><style>"+n+q+"</style></head><body>"+d+"</body></html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_"+f+".html")):(b=new Blob(["<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>\n"+t+"\n"+window.atob("")+
"<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script>\n</head><body>"+d+"</body></html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_Pfad_"+f+".html"))}});$(document).on("click","#zeigeSlideshow",function(){L(!1)});$(document).on("click","#speichereSlideshow",function(){L(!0)});$("#druckansicht").click(function(){r();var a="",b=0,c=0,f=0,d=0,g=!1;$(".tshareElement").each(function(){f++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&d++})});0<f&&0==d&&(g=!0);$(".tshareElement").each(function(){var e=!1,
p="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",f=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){e=$(this).hasClass("aktiv")});1==g&&(e=!0);1==e&&($(this).hasClass("zeichenflaeche")?a+="<div "+p+f+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n":$(this).hasClass("markdown")?(a+="<div "+p+f+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",b++):$(this).hasClass("datei")?
(c++,a+="<div "+p+f+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):$(this).hasClass("toc")?(a+="<div "+p+f+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n",b++):$(this).hasClass("qrcode")&&(a+="<div "+p+f+" class='qrcode tshareElement adoccss'>"+$(this).html()+"</div>\n",b++))});var h="",k="";0<b?(h=x(g),k=$("#asciidoctorstyle").text()):0<c&&(k=$("#asciidoctorstyle").text());var l=window.open();l.document.open();l.document.write("<!DOCTYPE html><html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+
window.atob("")+
"<script>"+window.atob("")+
"\x3c/script><script>\n"+window.atob("")+
"\x3c/script><script>"+$("#jqueryminified").text()+"\x3c/script>\n<script>"+window.atob("")+
"\x3c/script><style>"+h+k+"</style></head><body>"+a+"</body></html>")});$(document).on("click",".menu-position",function(){var a=$(this).attr("position");$(".transparent").remove();switch(a){case "left":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuLeft");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveLeft");$("[position='left']").hide();$("[position='right']").show();
$("[position='top']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "right":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#header").addClass("menuRight");$("[position='right']").hide();$("[position='left']").show();$("[position='top']").show();$("[position='bottom']").show();
$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "bottom":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveUp");$("[position='bottom']").hide();$("[position='top']").show();$("[position='right']").show();$("[position='left']").show();$(".auswahl").removeClass("selectMenu");
break;default:B()}});$(document).on("click","#recalculate",function(){r();nerdamer.flush();nerdamer.clearVars();var a=[];$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>Bitte warten ...</div>");var b=0,c=0,f=!1;$(".tshareElement").each(function(){b++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&c++})});0<b&&0==c&&(f=!0);$(".markdownText").each(function(){var b=$(this).children(":first").next().text();if(1==f||$(this).parent(".tshareElement").next().find(".markieren").hasClass("aktiv")){$(this).children(":first").attr("id",
"recalculationDiv");b=Textabschnitt.recalculate(b,"#recalculationDiv");if(null!=b&&0<b.length)for(var c=0;c<b.length;c++)a.push(b[c]);$(this).children(":first").removeAttr("id")}});for(var d="\n== Inhaltsverzeichnis\n\n",g=0;g<a.length;g++){for(var d=d+"[none]\n",h=0;h<a[g][0]-1;h++)d+="*";d+=" <<"+a[g][1]+","+a[g][2]+">>\n"}d+="\n\n---";g=Opal.hash2(["header_footer","attributes"],{header_footer:!1,attributes:["icons=font"]});d=Opal.Asciidoctor.$convert(d,g);$("#toc").remove();$("#tocBar").remove();
$("#firstMenu").after("<div class='tshareElement toc adoccss' id='toc'><div><div class='adoccss'>"+d+"</div></div></div>"+E);$(".menuBar").first().attr("id","tocBar");$(".transparent").remove();m()});$(document).on("click",".neueZeichenflaeche",function(){r();$(this).parent().after("<div class='zeichenflaeche' id='zeichnung'></div>"+E);bearbeitungAn=!0;z="zeichenflaeche";n=998;k=701;QreatorBezier.init("#zeichnung",998,701,"#menu",!0);m()});$(document).on("click",".markieren",function(){r();var a=
(new Date).getTime(),b=$(this).attr("lastClick");$(this).attr("lastClick",a);if(""==b||null==b)b=""+a;b=parseInt(b);if(1E3>a-b&&0==D)$(this).removeClass("aktiv"),$(this).addClass("bereich"),D=!0;else if($(this).hasClass("bereich")&&1==D)D=!1,$(this).removeClass("bereich"),$(this).removeClass("aktiv");else if(1!=D||$(this).hasClass("bereich"))$(this).toggleClass("aktiv");else{var c=-1,f=-1;$(this).addClass("endeBereich");$(this);var d=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("endeBereich")?
(f=d,$(this).removeClass("endeBereich")):$(this).hasClass("bereich")&&(c=d);d++});d=0;c<f?$(".markieren").not(".versteckt").each(function(){d>=c&&d<=f&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),$(this).addClass("aktiv"),D=!1);d++}):c>f&&$(".markieren").not(".versteckt").each(function(){d>=f&&d<=c&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),D=!1);d++})}m()});$("#markiereAlle").click(function(){r();D=!1;var a=0;$(".tshareElement").not(".versteckt").each(function(){a++});
var b=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("aktiv")&&b++});var c=(new Date).getTime();b<a?($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").addClass("aktiv").removeClass("bereich");$(this).attr("lastClick",c)}),m()):($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").removeClass("bereich")}),$(".wahl").each(function(){$(this).removeClass("komplett")}))});$("#auswahlLoeschen").click(function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlten Abschnitte inklusive aller Inhalte entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&
(r(),$(".markieren").each(function(){$(this).hasClass("aktiv")&&($(this).parent().prev().remove(),$(this).parent().remove())}));m()});$(document).on("click",".einfuegenClipboard",function(){$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><p>Bitte f\u00fcgen Sie den Inhalt des Clipboards in das Textfeld ein (z. B. per Strg+V) und dr\u00fccken Sie dann den Knopf \"Einf\u00fcgen\"</p><textarea id='clipboardExtern' required='required' ></textArea><button id='clipboardExternEinfuegen'>Einf\u00fcgen in Dokument</button><button id='abbrechenEinsetzen'>Abbrechen</button></div>");
$("#clipboardExtern").focus()});$(document).on("click","#abbrechenEinsetzen",function(){$(".transparent").remove();$("einsetzen").remove()});$(document).on("click","#clipboardExternEinfuegen",function(){var a=$("#clipboardExtern").val();a.startsWith("JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F")?1==confirm("Sind Sie sicher, dass dieser Abschnitt eingef\u00fcgt werden soll? Dieser Schritt kann nicht mehr r\u00fcckg\u00e4ngig gemacht werden!")&&(r(),$("#einsetzen").after(a.substring(30,a.length)),
$(".transparent").remove(),$("#einsetzen").remove(),m()):window.alert("Der Inhalt hat das falsche Format. Es k\u00f6nnen nur Inhalte aus Tshare eingef\u00fcgt werden.")});$(document).on("click",".einfuegen",function(){r();$(this).parent().after(K);m()});$(document).on("click",".markdownEinfuegen",function(){r();$(this).parent().after("<div class='markdown tshareElement' id='markdown'></div>"+M);bearbeitungAn=!0;z="markdown";Textabschnitt.init("#markdown","#menu");m()});$(document).on("click",".changeSeparator",
function(){r();var a=0,b=0,b=parseInt($(this).parent().prev(".tshareElement").attr("status"));if(void 0==b||isNaN(b))b=0;a=(b+1)%3;$(this).parent().prev(".datei").find("hr").each(function(){$(this).attr("class","");$(this).addClass("style_"+a)});$(this).parent().prev(".tshareElement").removeAttr("status");$(this).parent().prev(".tshareElement").attr("status",a);$(this).removeClass("statusFarbeKnopf_"+b);$(this).addClass("statusFarbeKnopf_"+a)});$(document).on("click",".bildEinfuegen",function(){r();
$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='bildInput' type='file' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");$("#bildInput").trigger("click")});this.saveObject=function(){for(var a=window.event.srcElement,b=a.previousSibling.getAttribute("data"),c=a.previousSibling.getAttribute("type"),a=a.previousSibling.getAttribute("name"),
f=b.indexOf(","),b=atob(b.substring(f+1)),f=[],d=0;d<b.length;d+=512){for(var g=b.slice(d,d+512),h=Array(g.length),k=0;k<g.length;k++)h[k]=g.charCodeAt(k);g=new Uint8Array(h);f.push(g)}c=new Blob(f,{type:c});saveAs(c,a)};$(document).on("change","#bildInput",function(){var a=$("#bildInput")[0].files[0],b=new FileReader;b.onload=function(b){$("#bildInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");var c=new Date,c=c.getDate()+"."+(c.getMonth()+1)+"."+c.getFullYear()+
", "+c.getHours()+":"+("0"+c.getMinutes()).substr(-2)+":"+("0"+c.getSeconds()).substr(-2),c="|===\n.3+|icon:file-o[5x]|Dateiname|"+a.name.split(" ").join("_")+"\n|Gr\u00f6\u00dfe in Bytes|"+a.size+"|Einf\u00fcgezeitpunkt|"+c+"\n|===",e=Opal.hash2(["header_footer","attributes"],{header_footer:!1,attributes:["icons=font"]}),c=Opal.Asciidoctor.$convert(c,e);$("#einsetzen").before("<div class='datei tshareElement'><div style='background-color: #f8f8f7'><object style='display:none' id='dataNeu' data='' name="+
a.name.split(" ").join("_")+" type='"+a.type+"'></object><button onclick='saveObject()'>"+a.name.split(" ").join("_")+" herunterladen</button><div class='adoccss'>"+c+"</div></div></div>"+E);$("#dataNeu").attr("data",b.target.result);$("#dataNeu").removeAttr("id");$(".transparent").remove();$("#einsetzen").remove()};b.readAsDataURL(a);m()});$(document).on("click",".downloadFile",function(){var a=$(this).attr("data"),a=Base64Binary.decodeArrayBuffer(a);new Blob([a],{type:$(this).attr("datatype")});
saveAs([a],$(this).attr("filename"))});$(document).on("click",".pdf",function(){r();$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='pdfInput' type='file' accept='application/pdf' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");$("#pdfInput").trigger("click")});$(document).on("change","#pdfInput",function(){var a=$("#pdfInput")[0].files[0],
b=new FileReader;b.onload=function(b){if(a.name.endsWith(".pdf")){var c=function(a){that.seitenZaehler++;var b=a.getViewport(1),b=a.getViewport(2*n/b.width),f=document.createElement("canvas");f.style.display="block";var l=f.getContext("2d");f.height=b.height;f.width=b.width;a=a.render({canvasContext:l,viewport:b});var m=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){m.call(this,a);this.seitennummer=this.pageNumber;var b=this;a=l.getImageData(0,0,f.width,f.height);for(var c=
0,e=a.data.length;c<e;c+=4){var p=a.data[c+1],q=a.data[c+2];255==a.data[c]&&255==p&&255==q&&(a.data[c+3]=0)}l.putImageData(a,0,0);y=new Image;y.onload=function(){g=f.width;h=f.height;that.zaehler++;QreatorBezier.insertImgAsSVG(this,n,k,"einsetzen"+b.seitennummer,E);console.log("Z\u00e4hler "+that.zaehler+" Seitennummer: "+b.seitennummer);that.zaehler==d&&($(".einsetzen").remove(),$(".transparent").remove())};y.src=f.toDataURL()};e++;null!==p&&e<=d&&p.getPage(e).then(c)};$("#pdfInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");
PDFJS.disableWorker=!0;var e=1,d=0,p=null;this.bildLadeZaehler=this.seitenZaehler=this.zaehler=0;that=this;PDFJS.getDocument(b.target.result).then(function(a){p=a;d=a.numPages;for(var b=0;b<d;b++)$("#einsetzen").before("<div class='einsetzen' id='einsetzen"+(b+1)+"'></div>");$("#einsetzen").remove();a.getPage(1).then(c)})}};b.readAsDataURL(a);m()});$(document).on("click",".zeichenflaecheKlick",function(){r();var a=$(this).html();$(this).removeClass("zeichenflaecheKlick");bearbeitungAn=!0;z="zeichenflaeche";
var b=parseInt($(this).find("svg").attr("width")),c=parseInt($(this).find("svg").attr("height"));QreatorBezier.init(this,b,c,"#menu",!0);QreatorBezier.loadSVG(a)});$(document).on("click",".markdownText",function(){r();var a=$(this).find(".originaltext").text();$(this).parent().attr("id","markdown");$(this).parent().removeClass("adoccss");bearbeitungAn=!0;z="markdown";Textabschnitt.init("#markdown","#menu",a)});$(document).on("change","#fileInput",function(){var a=$("#fileInput")[0].files[0],b=new FileReader;
b.onload=function(a){a=a.target.result;var b=document.createElement("div");b.id="container";b.innerHTML=a;$("#content").html("<span id='firstMenu'><button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageFotoNeu unten'></button></span>");
$(b).find(".tshareElement").each(function(){var a=$(this).attr("sb");if(null==a||""==a)a="@";var b=$(this).attr("ft");if(null==b||""==b)b="=";$(this).attr("tags");$(this).hasClass("zeichenflaeche")?($("#content").append("<div class='zeichenflaeche tshareElement zeichenflaecheKlick'>"+$(this).html()+"</div>"),$("#content").append(E)):$(this).hasClass("markdown")?($("#content").append("<div class='markdown tshareElement adoccss'>"+$(this).html()+"</div>"),$("#content").append(M)):$(this).hasClass("datei")?
($("#content").append("<div class='datei tshareElement'>"+$(this).html()+"</div>"),$("#content").append(E)):$(this).hasClass("toc")&&($("#content").append("<div id='toc' class='toc tshareElement'>"+$(this).html()+"</div>"),$("#content").append(E),$(".menuBar").last().attr("id","tocBar"));$(".ebenenAnzeige").last().attr("aktiv",a);$(".folientyp").last().val(b);a=H(a);$(".ebenenAnzeige").last().wrap("<span id='ersetzung'></span>");$("#ersetzung").html(a);$("#ersetzung .ebenenAnzeige").unwrap()});$(".transparent").remove();
m()};b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsText(a)});$(document).on("click","#laden",function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='fileInput' type='file' accept='text/html' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");$("#fileInput").trigger("click")});$(document).on("click","#bild",
function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='imageInput' type='file' accept='image/*' required='required' ></input><button id='abbrechenBild'>Abbrechen</button><div id='parameter'></div></div>");$("#imageInput").trigger("click")});$(document).on("click","#abbrechen",function(){$(".transparent").remove();$("#einsetzen").remove()});$(document).on("click","#abbrechenBild",function(){QreatorBezier.loescheBild();$(".transparent").remove()});
$(document).on("mousemove",".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*g/n*b/100));isNaN(b)&&(b=parseInt(100*h/k*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());$(".fenster #breite").html(""+a);$(".fenster #hoehe").html(""+b);$(".fenster #xpos").html(""+c);$(".fenster #ypos").html(""+d)});$(document).on("change",".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),
b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*g/n*b/100));isNaN(b)&&(b=parseInt(100*h/k*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(y,parseInt(c*n/100),parseInt(d*k/100),parseInt(a*n/100),parseInt(b*k/100),!1)});$(document).on("click","#bildEinfuegen",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=100*g/n*b/100);isNaN(b)&&(b=100*h/k*a/100);var c=parseInt($("[typ='x']").val()),
d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(y,parseInt(c*n/100),parseInt(d*k/100),parseInt(a*n/100),parseInt(b*k/100),!0);$(".transparent").remove()});$(document).on("click",".mittig",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*g/n*b/100));isNaN(b)&&(b=parseInt(100*h/k*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());switch($(this).attr("typ")){case "xmitte":c=parseInt(50-a/2);break;case "ymitte":d=
parseInt(50-b/2);break;case "links":c=0;break;case "rechts":c=100-a;break;case "oben":d=0;break;case "unten":d=100-b}$(".fenster #xpos").html(""+c);$("[typ='x']").val(c);$(".fenster #ypos").html(""+d);$("[typ='y']").val(d);QreatorBezier.ladeBild(y,parseInt(c*n/100),parseInt(d*k/100),parseInt(a*n/100),parseInt(b*k/100),!1)});$(document).on("change","#imageInput",function(){var a=$("#imageInput")[0].files[0],b=new FileReader;b.onload=function(b){a.name.endsWith(".pdf")?(console.log("PDF"),PDFJS.disableWorker=
!0,console.log(b.target.result),PDFJS.getDocument(b.target.result).then(function(a){a.getPage(1).then(function(a){var b=a.getViewport(2.5),c=document.createElement("canvas"),d=c.getContext("2d");c.height=b.height;c.width=b.width;a=a.render({canvasContext:d,viewport:b});var e=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){e.call(this,a);a=d.getImageData(0,0,c.width,c.height);for(var b=0,f=a.data.length;b<f;b+=4){var l=a.data[b+1],m=a.data[b+2];255==a.data[b]&&255==l&&255==
m&&(a.data[b+3]=0)}d.putImageData(a,0,0);y=new Image;y.onload=function(){g=c.width;h=c.height;w=!0;this.width>this.height&&(w=!1);1==w?(h=Math.min(k,this.height),h<k&&(h=k),g=this.width*h/this.height,g>n&&(h=h*n/g,g=n,w=!1)):(g=Math.min(n,this.width),g<n&&(g=n),h=this.height*g/this.width,h>k&&(g=g*k/h,h=k,w=!0));console.log(y.src);0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*
g/n)+"'></input><span id='breite'>"+parseInt(100*g/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+parseInt(100*h/k)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*h/k)+"'></input><span id='hoehe'>"+parseInt(100*h/k)+"</span>%, Breite: <span id='breite'>"+parseInt(100*g/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(y,0,0,g,h,!1)};y.src=c.toDataURL()}})})):
(y=new Image,y.onload=function(){h=g=0;w=!0;this.width>this.height&&(w=!1);1==w?(h=Math.min(k,this.height),h<k&&(h=k),g=this.width*h/this.height,g>n&&(h=h*n/g,g=n,w=!1)):(g=Math.min(n,this.width),g<n&&(g=n),h=this.height*g/this.width,h>k&&(g=g*k/h,h=k,w=!0));0==w?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*g/n)+"'></input><span id='breite'>"+parseInt(100*g/n)+"</span>%, H\u00f6he:<span id='hoehe'>"+
parseInt(100*h/k)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*h/k)+"'></input><span id='hoehe'>"+parseInt(100*h/k)+"</span>%, Breite: <span id='breite'>"+parseInt(100*g/n)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(y,0,0,g,h,!1)},y.src=b.target.result)};
b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsDataURL(a)});$(document).on("click",".loeschen",function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlte Zeichenfl\u00e4che inklusive Inhalt entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&(r(),$(this).parent().prev().remove(),$(this).parent().remove(),m())})});
