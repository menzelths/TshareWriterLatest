var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.findInternal=function(a,c,b){a instanceof String&&(a=String(a));for(var l=a.length,n=0;n<l;n++){var d=a[n];if(c.call(b,d,n,a))return{i:n,v:d}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(a,c,b){a!=Array.prototype&&a!=Object.prototype&&(a[c]=b.value)};
$jscomp.getGlobal=function(a){return"undefined"!=typeof window&&window===a?a:"undefined"!=typeof global&&null!=global?global:a};$jscomp.global=$jscomp.getGlobal(this);$jscomp.polyfill=function(a,c,b,l){if(c){b=$jscomp.global;a=a.split(".");for(l=0;l<a.length-1;l++){var n=a[l];n in b||(b[n]={});b=b[n]}a=a[a.length-1];l=b[a];c=c(l);c!=l&&null!=c&&$jscomp.defineProperty(b,a,{configurable:!0,writable:!0,value:c})}};
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,b){return $jscomp.findInternal(this,a,b).v}},"es6-impl","es3");$jscomp.polyfill("Object.is",function(a){return a?a:function(a,b){return a===b?0!==a||1/a===1/b:a!==a&&b!==b}},"es6-impl","es3");$jscomp.polyfill("Array.prototype.includes",function(a){return a?a:function(a,b){var c=this;c instanceof String&&(c=String(c));var n=c.length;for(b=b||0;b<n;b++)if(c[b]==a||Object.is(c[b],a))return!0;return!1}},"es7","es3");
$jscomp.checkStringArgs=function(a,c,b){if(null==a)throw new TypeError("The 'this' value for String.prototype."+b+" must not be null or undefined");if(c instanceof RegExp)throw new TypeError("First argument to String.prototype."+b+" must not be a regular expression");return a+""};$jscomp.polyfill("String.prototype.includes",function(a){return a?a:function(a,b){return-1!==$jscomp.checkStringArgs(this,a,"includes").indexOf(a,b||0)}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var n=c.length,d=a.length;b=Math.max(0,Math.min(b|0,c.length));for(var u=0;u<d&&b<n;)if(c[b++]!=a[u++])return!1;return u>=d}},"es6-impl","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};$jscomp.symbolCounter_=0;
$jscomp.Symbol=function(a){return $jscomp.SYMBOL_PREFIX+(a||"")+$jscomp.symbolCounter_++};$jscomp.initSymbolIterator=function(){$jscomp.initSymbol();var a=$jscomp.global.Symbol.iterator;a||(a=$jscomp.global.Symbol.iterator=$jscomp.global.Symbol("iterator"));"function"!=typeof Array.prototype[a]&&$jscomp.defineProperty(Array.prototype,a,{configurable:!0,writable:!0,value:function(){return $jscomp.arrayIterator(this)}});$jscomp.initSymbolIterator=function(){}};
$jscomp.arrayIterator=function(a){var c=0;return $jscomp.iteratorPrototype(function(){return c<a.length?{done:!1,value:a[c++]}:{done:!0}})};$jscomp.iteratorPrototype=function(a){$jscomp.initSymbolIterator();a={next:a};a[$jscomp.global.Symbol.iterator]=function(){return this};return a};$jscomp.makeIterator=function(a){$jscomp.initSymbolIterator();var c=a[Symbol.iterator];return c?c.call(a):$jscomp.arrayIterator(a)};$jscomp.owns=function(a,c){return Object.prototype.hasOwnProperty.call(a,c)};
$jscomp.polyfill("WeakMap",function(a){function c(a){$jscomp.owns(a,l)||$jscomp.defineProperty(a,l,{value:{}})}function b(a){var b=Object[a];b&&(Object[a]=function(a){c(a);return b(a)})}if(function(){if(!a||!Object.seal)return!1;try{var b=Object.seal({}),y=Object.seal({}),c=new a([[b,2],[y,3]]);if(2!=c.get(b)||3!=c.get(y))return!1;c.delete(b);c.set(y,4);return!c.has(b)&&4==c.get(y)}catch(F){return!1}}())return a;var l="$jscomp_hidden_"+Math.random().toString().substring(2);b("freeze");b("preventExtensions");
b("seal");var n=0,d=function(a){this.id_=(n+=Math.random()+1).toString();if(a){$jscomp.initSymbol();$jscomp.initSymbolIterator();a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};d.prototype.set=function(a,b){c(a);if(!$jscomp.owns(a,l))throw Error("WeakMap key fail: "+a);a[l][this.id_]=b;return this};d.prototype.get=function(a){return $jscomp.owns(a,l)?a[l][this.id_]:void 0};d.prototype.has=function(a){return $jscomp.owns(a,l)&&$jscomp.owns(a[l],this.id_)};d.prototype.delete=
function(a){return $jscomp.owns(a,l)&&$jscomp.owns(a[l],this.id_)?delete a[l][this.id_]:!1};return d},"es6-impl","es3");$jscomp.MapEntry=function(){};
$jscomp.polyfill("Map",function(a){if(!$jscomp.ASSUME_NO_NATIVE_MAP&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([[b,"s"]]));if("s"!=c.get(b)||1!=c.size||c.get({x:4})||c.set({x:4},"t")!=c||2!=c.size)return!1;var d=c.entries(),l=d.next();if(l.done||l.value[0]!=b||"s"!=l.value[1])return!1;l=d.next();return l.done||4!=l.value[0].x||"t"!=l.value[1]||!d.next().done?!1:!0}catch(C){return!1}}())return a;$jscomp.initSymbol();
$jscomp.initSymbolIterator();var c=new WeakMap,b=function(a){this.data_={};this.head_=d();this.size=0;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};b.prototype.set=function(a,b){var c=l(this,a);c.list||(c.list=this.data_[c.id]=[]);c.entry?c.entry.value=b:(c.entry={next:this.head_,previous:this.head_.previous,head:this.head_,key:a,value:b},c.list.push(c.entry),this.head_.previous.next=c.entry,this.head_.previous=c.entry,this.size++);return this};b.prototype.delete=
function(a){a=l(this,a);return a.entry&&a.list?(a.list.splice(a.index,1),a.list.length||delete this.data_[a.id],a.entry.previous.next=a.entry.next,a.entry.next.previous=a.entry.previous,a.entry.head=null,this.size--,!0):!1};b.prototype.clear=function(){this.data_={};this.head_=this.head_.previous=d();this.size=0};b.prototype.has=function(a){return!!l(this,a).entry};b.prototype.get=function(a){return(a=l(this,a).entry)&&a.value};b.prototype.entries=function(){return n(this,function(a){return[a.key,
a.value]})};b.prototype.keys=function(){return n(this,function(a){return a.key})};b.prototype.values=function(){return n(this,function(a){return a.value})};b.prototype.forEach=function(a,b){for(var c=this.entries(),d;!(d=c.next()).done;)d=d.value,a.call(b,d[1],d[0],this)};b.prototype[Symbol.iterator]=b.prototype.entries;var l=function(a,b){var d=b&&typeof b;"object"==d||"function"==d?c.has(b)?d=c.get(b):(d=""+ ++u,c.set(b,d)):d="p_"+b;var l=a.data_[d];if(l&&$jscomp.owns(a.data_,d))for(a=0;a<l.length;a++){var n=
l[a];if(b!==b&&n.key!==n.key||b===n.key)return{id:d,list:l,index:a,entry:n}}return{id:d,list:l,index:-1,entry:void 0}},n=function(a,b){var c=a.head_;return $jscomp.iteratorPrototype(function(){if(c){for(;c.head!=a.head_;)c=c.previous;for(;c.next!=c.head;)return c=c.next,{done:!1,value:b(c)};c=null}return{done:!0,value:void 0}})},d=function(){var a={};return a.previous=a.next=a.head=a},u=0;return b},"es6-impl","es3");
$jscomp.polyfill("Set",function(a){if(!$jscomp.ASSUME_NO_NATIVE_SET&&function(){if(!a||!a.prototype.entries||"function"!=typeof Object.seal)return!1;try{var b=Object.seal({x:4}),c=new a($jscomp.makeIterator([b]));if(!c.has(b)||1!=c.size||c.add(b)!=c||1!=c.size||c.add({x:4})!=c||2!=c.size)return!1;var n=c.entries(),d=n.next();if(d.done||d.value[0]!=b||d.value[1]!=b)return!1;d=n.next();return d.done||d.value[0]==b||4!=d.value[0].x||d.value[1]!=d.value[0]?!1:n.next().done}catch(u){return!1}}())return a;
$jscomp.initSymbol();$jscomp.initSymbolIterator();var c=function(a){this.map_=new Map;if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)this.add(b.value)}this.size=this.map_.size};c.prototype.add=function(a){this.map_.set(a,a);this.size=this.map_.size;return this};c.prototype.delete=function(a){a=this.map_.delete(a);this.size=this.map_.size;return a};c.prototype.clear=function(){this.map_.clear();this.size=0};c.prototype.has=function(a){return this.map_.has(a)};c.prototype.entries=function(){return this.map_.entries()};
c.prototype.values=function(){return this.map_.values()};c.prototype.keys=c.prototype.values;c.prototype[Symbol.iterator]=c.prototype.values;c.prototype.forEach=function(a,c){var b=this;this.map_.forEach(function(d){return a.call(c,d,d,b)})};return c},"es6-impl","es3");
$jscomp.polyfill("Array.from",function(a){return a?a:function(a,b,l){$jscomp.initSymbolIterator();b=null!=b?b:function(a){return a};var c=[],d=a[Symbol.iterator];if("function"==typeof d)for(a=d.call(a);!(d=a.next()).done;)c.push(b.call(l,d.value));else for(var d=a.length,u=0;u<d;u++)c.push(b.call(l,a[u]));return c}},"es6-impl","es3");
$jscomp.polyfill("String.prototype.endsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"endsWith");a+="";void 0===b&&(b=c.length);b=Math.max(0,Math.min(b|0,c.length));for(var n=a.length;0<n&&0<b;)if(c[--b]!=a[--n])return!1;return 0>=n}},"es6-impl","es3");
var saveAs=saveAs||function(a){if("undefined"===typeof navigator||!/MSIE [1-9]\./.test(navigator.userAgent)){var c=a.document.createElementNS("http://www.w3.org/1999/xhtml","a"),b="download"in c,l=/Version\/[\d\.]+.*Safari/.test(navigator.userAgent),n=a.webkitRequestFileSystem,d=a.requestFileSystem||n||a.mozRequestFileSystem,u=function(b){(a.setImmediate||a.setTimeout)(function(){throw b;},0)},y=0,B=function(b){var c=function(){"string"===typeof b?(a.URL||a.webkitURL||a).revokeObjectURL(b):b.remove()};
a.chrome?c():setTimeout(c,500)},F=function(a,b,c){b=[].concat(b);for(var d=b.length;d--;){var h=a["on"+b[d]];if("function"===typeof h)try{h.call(a,c||a)}catch(x){u(x)}}},G=function(a){return/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(a.type)?new Blob(["\ufeff",a],{type:a.type}):a},C=function(m,u,h){h||(m=G(m));var e=this;h=m.type;var v=!1,x=function(){F(e,["writestart","progress","write","writeend"])},r=function(){if(w&&l&&"undefined"!==typeof FileReader){var b=
new FileReader;b.onloadend=function(){var a=b.result;w.location.href="data:attachment/file"+a.slice(a.search(/[,;]/));e.readyState=e.DONE;x()};b.readAsDataURL(m);e.readyState=e.INIT}else{if(v||!q)q=(a.URL||a.webkitURL||a).createObjectURL(m);w?w.location.href=q:void 0==a.open(q,"_blank")&&l&&(a.location.href=q);e.readyState=e.DONE;x();B(q)}},f=function(a){return function(){if(e.readyState!==e.DONE)return a.apply(this,arguments)}},k={create:!0,exclusive:!1};e.readyState=e.INIT;u||(u="download");if(b){var q=
(a.URL||a.webkitURL||a).createObjectURL(m);setTimeout(function(){c.href=q;c.download=u;var a=new MouseEvent("click");c.dispatchEvent(a);x();B(q);e.readyState=e.DONE})}else{if(a.chrome&&h&&"application/octet-stream"!==h){var C=m.slice||m.webkitSlice;m=C.call(m,0,m.size,"application/octet-stream");v=!0}n&&"download"!==u&&(u+=".download");if("application/octet-stream"===h||n)var w=a;d?(y+=m.size,d(a.TEMPORARY,y,f(function(a){a.root.getDirectory("saved",k,f(function(a){var b=function(){a.getFile(u,k,
f(function(a){a.createWriter(f(function(g){g.onwriteend=function(g){w.location.href=a.toURL();e.readyState=e.DONE;F(e,"writeend",g);B(a)};g.onerror=function(){var a=g.error;a.code!==a.ABORT_ERR&&r()};["writestart","progress","write","abort"].forEach(function(a){g["on"+a]=e["on"+a]});g.write(m);e.abort=function(){g.abort();e.readyState=e.DONE};e.readyState=e.WRITING}),r)}),r)};a.getFile(u,{create:!1},f(function(a){a.remove();b()}),f(function(a){a.code===a.NOT_FOUND_ERR?b():r()}))}),r)}),r)):r()}},
v=C.prototype;if("undefined"!==typeof navigator&&navigator.msSaveOrOpenBlob)return function(a,b,c){c||(a=G(a));return navigator.msSaveOrOpenBlob(a,b||"download")};v.abort=function(){this.readyState=this.DONE;F(this,"abort")};v.readyState=v.INIT=0;v.WRITING=1;v.DONE=2;v.error=v.onwritestart=v.onprogress=v.onwrite=v.onabort=v.onerror=v.onwriteend=null;return function(a,b,c){return new C(a,b,c)}}}("undefined"!==typeof self&&self||"undefined"!==typeof window&&window||this.content);
"undefined"!==typeof module&&module.exports?module.exports.saveAs=saveAs:"undefined"!==typeof define&&null!==define&&null!=define.amd&&define([],function(){return saveAs});
$(function(){function a(a){for(var g="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(g+=String.fromCharCode(64+b));$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".ebenenAnzeige").each(function(){var t=$(this).attr("aktiv");if("plus"==a)for(var b=0;b<g.length;b++)t.includes(g.charAt(b))||(t+=g.charAt(b));else if("minus"==a){for(var p="",b=0;b<t.length;b++)g.includes(t.charAt(b))||(p+=t.charAt(b));t=p}else"gleich"==a&&(t=g);$(this).attr("aktiv",t);b=B(t);$(this).parent().prev(".tshareElement").attr("sb",
t);$(this).wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove()})})}function c(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv").addClass("aktiv")})}function b(a){$(".ebenenAnzeige").each(function(){$(this).parent().hasClass("versteckt")||$(this).attr("aktiv").toUpperCase().includes(String.fromCharCode(64+
parseInt(a)))&&$(this).parent().find(".markieren").removeClass("aktiv")})}function l(){for(var a=Array(27),t=Array(27),b=0;27>b;b++)a[b]=0,t[b]=0;$(".ebenenAnzeige").each(function(){if(!$(this).parent().hasClass("versteckt")){for(var g=$(this).attr("aktiv").toUpperCase(),b=0;b<g.length;b++)t[g.charCodeAt(b)-64]++;if($(this).parent().find(".markieren").hasClass("aktiv"))for(b=0;b<g.length;b++)a[g.charCodeAt(b)-64]++}});return[a,t]}function n(){$(".tshareElement").each(function(){var a=parseInt($(this).attr("sichtbarkeit"));
0==J[a]?$(this).attr("visibility","hidden"):$(this).removeAttr("visibility")})}function d(){for(var a=l(),b=a[0],a=a[1],p=0;27>p;p++)0<a[p]&&b[p]==a[p]?($("[wahl='"+p+"']").removeClass("komplett").addClass("komplett"),$("[wahl='"+p+"']").attr("style","display:true")):0<a[p]&&b[p]<a[p]?($("[wahl='"+p+"']").removeClass("komplett"),$("[wahl='"+p+"']").attr("style","display:true")):0==a[p]&&($("[wahl='"+p+"']").removeClass("komplett"),$("[wahl='"+p+"']").attr("style","display:none"))}function u(){$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");
$("#header").addClass("menuTop");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveDown");$("[position='top']").hide();$("[position='right']").show();$("[position='left']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu")}function y(){var a=new Date,b=a.getMonth()+1,p="";10>b&&(p="0");var p=p+b,c=a.getDate(),b="";10>c&&(b="0");var b=b+c,c="",d=a.getHours();10>d&&(c="0");var c=c+d,d="",e=a.getMinutes();10>e&&
(d="0");var d=d+e,e="",f=a.getSeconds();10>f&&(e="0");e+=f;return""+(1900+a.getYear())+p+b+"_"+c+d+e}function B(a){var b=document.createElement("canvas");a=a.toUpperCase();var g=b.getContext("2d");b.height=35;b.width=106;g.font="10px Sans-Serif";g.fillStyle="#ffff00";g.fillStyle="#000000";for(var c=0;27>c;c++)a.includes(String.fromCharCode(64+c))?(g.fillStyle="#ffffaa",g.fillRect(c%9*12,12*parseInt(c/9),12,12),g.fillStyle="#000000"):g.fillStyle="#aaaaaa",g.fillText(String.fromCharCode(64+c),2+c%9*
12,10+12*parseInt(c/9));g="";for(c=0;c<a.length;c++)g.includes(a.charAt(c))||(g+=a.charAt(c));return"<span class='ebenenAnzeige' aktiv='"+g+"'><img src='"+b.toDataURL()+"'   ></img></span>"}function F(a){for(var b="<div>",g=0;27>g;g++){var c="";a.includes(String.fromCharCode(64+g))&&(c="aktiv");b+="<button id='bs_"+g+"' class='ebenenknopf "+c+"'>"+String.fromCharCode(64+g)+"</button>";0==(g+1)%9&&(b+="<br>")}return b+"</div>"}function G(a){for(var g="",b=".katex .delimsizing.size1;.katex .delimsizing.size2;.katex .delimsizing.size3;.katex .delimsizing.size4;.katex .delimsizing.mult .delim-size1 > span;.katex .delimsizing.mult .delim-size4 > span;.katex .op-symbol.small-op;.katex .op-symbol.large-op;.katex .mathit;.katex .mathbf;.katex .amsrm;.katex .mathbb;.katex .mathcal;.katex .mathfrak;.katex .mathtt;.katex .mathscr;.katex .mathsf;.katex .mainit".split(";"),
g="KaTeX_Size1 KaTeX_Size2 KaTeX_Size3 KaTeX_Size4 KaTeX_Size1 KaTeX_Size4 KaTeX_Size1 KaTeX_Size2 KaTeX_Main KaTeX_Main KaTeX_Math KaTeX_Main KaTeX_AMS KaTeX_AMS KaTeX_Caligraphic KaTeX_Fraktur KaTeX_Typewriter KaTeX_Script KaTeX_SansSerif KaTeX_Main".split(" "),c=0;c<b.length;c++){var d=0;$(b[c]).each(function(){var b=!1;$(this).parents(".tshareElement").next().find(".markieren").each(function(){b=$(this).hasClass("aktiv")});1!=b&&1!=a||d++});b[c]=d}for(var e=new Set,c=0;c<b.length;c++)0<b[c]&&
e.add(g[c]);0<e.size&&(e.add("KaTeX_Main"),e.add("KaTeX_Math"));b=Array.from(e);console.log("Schriften: ");for(var e="",f=$("#katexStyle").text().split("/* KateX_Fonts END */"),g=f[0].split("/**/"),c=0;c<b.length;c++)for(var k=b[c],h=0;h<g.length;h++)-1!=g[h].indexOf(k)&&(e+=g[h].trim());return g=e+f[1]}function C(a){var b=$("#dateiname").val();null!=b&&""==b&&(b="Slideshow");var g=y(),b=b+"_"+g+".html";$(".transparent").remove();var c="",d="",e="",f=0,k=0,h=0,l=0,n=!1,m=0;$(".tshareElement").each(function(){h++;
$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&l++})});0<h&&0==l&&(n=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",g=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){a=$(this).hasClass("aktiv")});var t="=";$(this).next().find(".folientyp").each(function(){t=$(this).val()});"="==t?e=d="":"+"==t?(0==m?d="<section>":1==m?d="</section><section>":
2==m&&(d="</section></section><section>"),e="",m=1):"++"==t?(0==m?d="<section><section>":1==m?d="</section><section><section>":2==m&&(d="</section><section>"),m=2,e=""):"/"==t&&(d="<span class='fragment'>",e="</span>");1==n&&(a=!0);1==a&&($(this).hasClass("zeichenflaeche")?c+=d+"<div "+b+g+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n"+e:$(this).hasClass("markdown")?(c+=d+"<div "+b+g+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n"+
e,f++):$(this).hasClass("datei")?(k++,c+=d+"<div "+b+g+" class='datei tshareElement'>"+$(this).html()+"</div>\n"+e):$(this).hasClass("toc")&&(c+=d+"<div "+b+g+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n"+e,f++))});1==m?c+="</section>":2==m&&(c+="</section></section>");var q=g="";0<f?(g=G(n),q=$("#asciidoctorstyle").text()):0<k&&(q=$("#asciidoctorstyle").text());0==a?(b=window.open(),b.document.open(),b.document.write("<!DOCTYPE html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+
window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+g+q+window.atob("")+
window.atob("")+
"</style></head><body><div class='reveal'><div class='slides'>"+c+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>")):(a=new Blob(["<!DOCTYPE html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+g+q+window.atob("")+
window.atob("")+
"</style></head><body><div class='reveal'><div class='slides'>"+c+"</div></div><script>"+window.atob("")+
"\x3c/script><script>Reveal.initialize();\x3c/script></body></html>"],{type:"text/html;charset=utf-8"}),saveAs(a,b))}function v(){var a="";m();$(".markieren").each(function(){if($(this).hasClass("aktiv")){var b=$(this).parent().prev(),g="";b.hasClass("markdown")?g="markdown adoccss":b.hasClass("zeichenflaeche")?g="zeichenflaeche zeichenflaecheKlick":b.hasClass("datei")&&(g="datei");a+="<div class='tshareElement "+g+"'>"+$(this).parent().prev().html()+"</div>"+A}});H=a;return a="JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+
a}function m(){1==bearbeitungAn&&("zeichenflaeche"==D?(QreatorBezier.showSVG("zeichenflaeche"),bearbeitungAn=!1):"markdown"==D&&(Textabschnitt.zeigeMarkdown("#markdownTable"),bearbeitungAn=!1,$("#markdown").addClass("adoccss"),$("#markdown").removeAttr("id")))}u();var J=[1,0,1,1,1,1,1,1,1,1],h=998,e=701;bearbeitungAn=!1;var D="",x="",r="",f=0,k=0,q=!1,H="",w=!1,E="";$(document).on("click",".wahl",function(){m();$(this).hasClass("komplett")?(b($(this).attr("wahl")),$(this).removeClass("komplett")):
(c($(this).attr("wahl")),$(this).removeClass("komplett"),$(this).addClass("komplett"));d()});$(document).on("click","#ebenenzuweisung",function(){$("body").append("<div class='transparent'></div>");var a=F("");$(".transparent").append("<div class='fenster'>"+a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneZuweisungPlus'>Tags hinzuf\u00fcgen</button><button id='ebeneZuweisungMinus'>Tags abziehen</button><button id='ebeneZuweisungGleich'>Tags \u00fcbernehmen</button></br><button id='ebeneAbbrechen'>Abbrechen</button></div>")});
$(document).on("click","#ebeneZuweisungPlus",function(){a("plus");d();$(".transparent").remove()});$(document).on("click","#ebeneZuweisungMinus",function(){a("minus");d();$(".transparent").remove()});$(document).on("click","#ebeneZuweisungGleich",function(){a("gleich");d();$(".transparent").remove()});for(var E="<select class='sichtbarkeit'>",z=0;27>z;z++)E=0==z?E+("<option selected sichtbarkeit='"+z+"'>"+String.fromCharCode(64+z)+"</option> "):E+("<option sichtbarkeit='"+z+"'>"+String.fromCharCode(64+
z)+"</option> ");E+="</select>";$(document).on("change",".folientyp",function(a){a.preventDefault();a=$(this).val();$(this).parent().prev(".tshareElement").attr("ft",a)});$(document).on("change",".auswahlebene",function(a){a.preventDefault();var b=parseInt($(this).prop("selectedIndex"));0<b&&$(".markieren").each(function(){$(this).hasClass("aktiv")&&$(this).parent().find(".sichtbarkeit").val(""+String.fromCharCode(64+b))});$(this).val("-");d()});$(document).on("change",".sichtbarkeit",function(a){a.preventDefault();
a=parseInt($(this).prop("selectedIndex"));$(this).parent().prev().attr("sichtbarkeit",a);n();d()});var z=B("@"),A="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select>"+z+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button></span>",
I="<span class='menuBar'><button class='loeschen imageTonne oben'></button><button class='markieren imageSelect oben' lastClick='0'></button><button class='kopiereAbschnitt imageCopy oben'></button><select class='folientyp'><option selected folientyp='0'>=</option><option folientyp='1'>+</option><option folientyp='2'>++</option><option folientyp='3'>/</option></select><button class='imagePen editPart oben'></button>"+z+"<button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageOpen unten'></button></span>";
$(document).on("click","#hideShow",function(){m();$(this).hasClass("komplett")?($(".tshareElement").each(function(){$(this).attr("style","display:true");$(this).removeClass("versteckt")}),$(".menuBar").each(function(){$(this).attr("style","display:true");$(this).removeClass("versteckt");$(this).find(".markieren").removeClass("versteckt")}),$(this).removeClass("komplett")):($(".markieren").each(function(){$(this).hasClass("aktiv")||($(this).parent().prev(".tshareElement").attr("style","display:none"),
$(this).parent().attr("style","display:none"),$(this).parent().removeClass("versteckt").addClass("versteckt"),$(this).removeClass("versteckt").addClass("versteckt"),$(this).parent().prev(".tshareElement").removeClass("versteckt").addClass("versteckt"))}),$(this).addClass("komplett"));d()});$(document).ready(function(){new Clipboard("#kopieren",{text:function(a){return v()}});new Clipboard(".kopiereAbschnitt",{text:function(a){m();var b="",g=$(a).parent().prev();g.hasClass("markdown")?b="<div class='tshareElement markdown adoccss'>"+
$(a).parent().prev().html()+"</div>"+I:g.hasClass("zeichenflaeche")?b="<div class='tshareElement zeichenflaeche zeichenflaecheKlick'>"+$(a).parent().prev().html()+"</div>"+A:g.hasClass("datei")&&(b="<div class='tshareElement datei'>"+$(a).parent().prev().html()+"</div>"+A);H=b;return"JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F"+b}});E="";for(var a=0;27>a;a++)E+="<button class='wahl allgemein knopfStandard' wahl='"+a+"' >"+String.fromCharCode(64+a)+"</button>";$("#ebenenwahl").html(E);$(".tshareElement").each(function(){var a=
$(this).attr("ft");if(null==a||""==a)a="=";$(this).next().find(".folientyp").val(a)});d()});window.addEventListener("beforeunload",function(a){return a.returnValue="Eventuelle \u00c4nderungen am Dokument k\u00f6nnten verloren gehen!"});document.addEventListener("paste",function(a){if(1==$("#rahmen").length&&0==$(".transparent").length)if(a=a.clipboardData.items)for(var b=0;b<a.length;b++){var g=a[b];if(g.type.startsWith("image/")){var g=g.getAsFile(),c=new FileReader;c.onload=function(a){r=new Image;
r.onload=function(){k=f=0;q=!0;this.width>this.height&&(q=!1);1==q?(k=Math.min(e,this.height),k<e&&(k=e),f=this.width*k/this.height,f>h&&(k=k*h/f,f=h,q=!1)):(f=Math.min(h,this.width),f<h&&(f=h),k=this.height*f/this.width,k>e&&(f=f*e/k,k=e,q=!0));$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><div id='parameter'></div><button id='abbrechenBild'>Abbrechen</button></div>");0==q?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+
parseInt(100*f/h)+"'></input><span id='breite'>"+parseInt(100*f/h)+"</span>%, H\u00f6he:<span id='hoehe'>"+parseInt(100*k/e)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*k/e)+"'></input><span id='hoehe'>"+parseInt(100*k/e)+"</span>%, Breite: <span id='breite'>"+parseInt(100*f/h)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,f,k,!1)};r.src=a.target.result};
c.onerror=function(a){console.log(a.target.error.name)};c.onprogress=function(a){console.log(a.loaded,a.total)};c.readAsDataURL(g)}}else console.log("fileList is null.")},!1);$("#speichern").click(function(){m();$("body").append("<div class='transparent'></div>");var a=x;""==x&&(a="Notiz");$(".transparent").append("<div class='fenster'>Speichern der aktuellen Datei (Zeitstempel und Endung werden angeh\u00e4ngt)<br>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><input type='checkbox' id='checkEditor'>Editor in Datei einbetten</input><br><button id='speichereDatei'>Speichern</button><button id='abbrechen'>Abbrechen</button></div>")});
$("#slideshow").click(function(){m();$("body").append("<div class='transparent'></div>");var a=x;""==x&&(a="Slideshow");$(".transparent").append("<div class='fenster'>Speichern / Zeigen der aktuellen Slideshow (Zeitstempel und Endung werden beim Speichern angeh\u00e4ngt)<br>Dateiname: <input type='text' id='dateiname' value='"+a+"'></input><br><button id='speichereSlideshow'>Speichern</button><button id='zeigeSlideshow'>Zeigen</button><button id='abbrechen'>Abbrechen</button></div>")});$("#menuansicht").click(function(){$("body").append("<div class='transparent'></div>");
$(".transparent").append("<div class='fenster'>Position des Men\u00fcs w\u00e4hlen:<br><button class='menu-position' position='left'>Men\u00fc links</button><button class='menu-position' position='top'>Men\u00fc oben</button><button class='menu-position' position='bottom'>Men\u00fc unten</button><button class='menu-position' position='right'>Men\u00fc rechts</button><br><button id='abbrechen'>Abbrechen</button></div>");$(".fenster").append('<p>Info:<br><div>Icons made by <a href="http://www.freepik.com" title="Freepik">Freepik</a>, <a href="http://www.flaticon.com/authors/yannick" title="Yannick">Yannick</a>, <a href="http://www.flaticon.com/authors/picol" title="Picol">Picol</a>, <a href="http://www.flaticon.com/authors/situ-herrera" title="Situ Herrera">Situ Herrera</a>, <a href="https://www.flaticon.com/authors/google" title="Google">Google</a>, <a href="http://www.flaticon.com/authors/freepik">freepik</a>, <a href="http://www.flaticon.com/authors/egor-rumyantsev" title="Egor Rumyantsev">Egor Rumyantsev</a>, <a href="http://fontawesome.io/" title="Dave Gandy">Dave Gandy</a>, <a href="https://www.flaticon.com/authors/appzgear" title="Appzgear">Appzgear</a>, <div>Icons made by <a href="https://www.flaticon.com/authors/dave-gandy" title="Dave Gandy">Dave Gandy</a> from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> licensed by <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0">CC BY 3.0</a></div>')});
$(document).on("click",".ebenenknopf",function(){$(this).toggleClass("aktiv")});$(document).on("click","#ebeneOK",function(){for(var a="",b=0;27>b;b++)$("#bs_"+b).hasClass("aktiv")&&(a+=String.fromCharCode(64+b));b=B(a);$("#ebenenAenderung").parent().prev(".tshareElement").attr("sb",a);$("#ebenenAenderung").wrap("<span id='ersetzung'></span>");$("#ersetzung").html(b);$("#ersetzung .ebenenAnzeige").unwrap();$(".transparent").remove();d()});$(document).on("click","#ebeneAbbrechen",function(){$("#ebenenAenderung").removeAttr("id");
$(".transparent").remove()});$(document).on("click","#ebeneAlle",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv").addClass("aktiv")});$(document).on("click","#ebeneKeine",function(){for(var a=0;27>a;a++)$("#bs_"+a).removeClass("aktiv")});$(document).on("click","#ebeneInvertiere",function(){for(var a=0;27>a;a++)$("#bs_"+a).toggleClass("aktiv")});$(document).on("click",".ebenenAnzeige",function(){$(this).attr("id","ebenenAenderung");$("body").append("<div class='transparent'></div>");
var a=F($(this).attr("aktiv"));$(".transparent").append("<div class='fenster'>"+a+"<br><button id='ebeneAlle'>Alle w\u00e4hlen</button><button id='ebeneKeine'>Alle abw\u00e4hlen</button><button id='ebeneInvertiere'>Auswahl invertieren</button><br><button id='ebeneOK'>OK</button><button id='ebeneAbbrechen'>Abbrechen</button></div>")});$(document).on("click",".editPart",function(){var a=$(this).parent().prev();if(a.hasClass("markdown")){m();var b=a.find(".originaltext").text();a.attr("id","markdown");
a.removeClass("adoccss");bearbeitungAn=!0;D="markdown";Textabschnitt.init("#markdown","#menu",b)}else if(a.hasClass("zeichenflaeche")){m();b=a.html();a.removeClass("zeichenflaecheKlick");bearbeitungAn=!0;D="zeichenflaeche";var c=parseInt(a.find("svg").attr("width")),d=parseInt(a.find("svg").attr("height"));QreatorBezier.init(a,c,d,"#menu",!0);QreatorBezier.loadSVG(b)}});$(document).on("click","#speichereDatei",function(){var a=$("#dateiname").val();var b=$("#checkEditor").prop("checked");if(null!=
a){""==a&&(a="Notiz");x=a;var c=y();$(".transparent").remove();if(1==b)b=new Blob(["<!DOCTYPE html>"+$("html").html()+"</html>"],{type:"text/html;charset=utf-8"}),saveAs(b,a+"_Editor_"+c+".html");else{var d="",e=0,f=0,k=0,h=0,l=!1;$(".tshareElement").each(function(){k++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&h++})});0<k&&0==h&&(l=!0);$(".tshareElement").each(function(){var a=!1,b="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",c=" ft='"+$(this).next().find(".folientyp :selected").text()+
"' ";$(this).next().find(".markieren").each(function(){a=$(this).hasClass("aktiv")});1==l&&(a=!0);1==a&&($(this).hasClass("zeichenflaeche")?d+="<div "+b+c+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n":$(this).hasClass("markdown")?(d+="<div "+b+c+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",e++):$(this).hasClass("datei")?(f++,d+="<div "+b+c+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):$(this).hasClass("toc")&&(d+=
"<div "+b+c+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n",e++))});var m=b="";0<e?(b=G(l),m=$("#asciidoctorstyle").text()):0<f&&(m=$("#asciidoctorstyle").text());b=new Blob(["<!DOCTYPE html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+b+m+"</style></head><body>"+d+"</body></html>"],{type:"text/html;charset=utf-8"});saveAs(b,a+"_"+c+".html")}}});$(document).on("click","#zeigeSlideshow",function(){C(!1)});$(document).on("click","#speichereSlideshow",function(){C(!0)});$("#druckansicht").click(function(){m();var a="",b=0,c=0,d=0,e=0,f=!1;$(".tshareElement").each(function(){d++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&e++})});0<d&&0==e&&(f=!0);$(".tshareElement").each(function(){var g=
!1,d="sb='"+$(this).next().find(".ebenenAnzeige").attr("aktiv")+"'",e=" ft='"+$(this).next().find(".folientyp :selected").text()+"' ";$(this).next().find(".markieren").each(function(){g=$(this).hasClass("aktiv")});1==f&&(g=!0);1==g&&($(this).hasClass("zeichenflaeche")?a+="<div "+d+e+" class='zeichenflaeche tshareElement' onClick='nextLayer(this)' >"+$(this).html()+"</div>\n":$(this).hasClass("markdown")?(a+="<div "+d+e+" class='markdown tshareElement adoccss'>"+$(this).html()+"</div>\n",b++):$(this).hasClass("datei")?
(c++,a+="<div "+d+e+" class='datei tshareElement'>"+$(this).html()+"</div>\n"):$(this).hasClass("toc")&&(a+="<div "+d+e+" class='toc tshareElement adoccss'>"+$(this).html()+"</div>\n",b++))});var k="",h="";0<b?(k=G(f),h=$("#asciidoctorstyle").text()):0<c&&(h=$("#asciidoctorstyle").text());var l=window.open();l.document.open();l.document.write("<!DOCTYPE html><head><meta name='description' content='Journal for HTML' ><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"+
window.atob("")+
"<script>"+window.atob("")+
"\n"+window.atob("")+
"\x3c/script><style>"+k+h+"</style></head><body>"+a+"</body></html>")});$(document).on("click",".menu-position",function(){var a=$(this).attr("position");$(".transparent").remove();switch(a){case "left":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuLeft");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveLeft");$("[position='left']").hide();$("[position='right']").show();
$("[position='top']").show();$("[position='bottom']").show();$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "right":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#header").addClass("menuRight");$("[position='right']").hide();$("[position='left']").show();$("[position='top']").show();$("[position='bottom']").show();
$(".auswahl").removeClass("selectMenu");$(".auswahl").addClass("selectMenu");break;case "bottom":$("#header").removeClass("menuLeft").removeClass("menuRight").removeClass("menuTop").removeClass("menuBottom");$("#header").addClass("menuBottom");$("#content").removeClass("moveLeft").removeClass("moveDown").removeClass("moveUp");$("#content").addClass("moveUp");$("[position='bottom']").hide();$("[position='top']").show();$("[position='right']").show();$("[position='left']").show();$(".auswahl").removeClass("selectMenu");
break;default:u()}});$(document).on("click","#recalculate",function(){m();nerdamer.flush();nerdamer.clearVars();var a=[];$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'>Bitte warten ...</div>");var b=0,c=0,e=!1;$(".tshareElement").each(function(){b++;$(this).next().find(".markieren").each(function(){$(this).hasClass("aktiv")&&c++})});0<b&&0==c&&(e=!0);$(".markdownText").each(function(){var b=$(this).children(":first").next().text();if(1==e||$(this).parent(".tshareElement").next().find(".markieren").hasClass("aktiv")){$(this).children(":first").attr("id",
"recalculationDiv");b=Textabschnitt.recalculate(b,"#recalculationDiv");if(null!=b&&0<b.length)for(var c=0;c<b.length;c++)a.push(b[c]);$(this).children(":first").removeAttr("id")}});for(var f="\n== Inhaltsverzeichnis\n\n",h=0;h<a.length;h++){for(var f=f+"[none]\n",k=0;k<a[h][0]-1;k++)f+="*";f+=" <<"+a[h][1]+","+a[h][2]+">>\n"}f+="\n\n---";h=Opal.hash2(["header_footer","attributes"],{header_footer:!1,attributes:["icons=font"]});f=Opal.Asciidoctor.$convert(f,h);$("#toc").remove();$("#tocBar").remove();
$("#firstMenu").after("<div class='tshareElement toc adoccss' id='toc'><div><div class='adoccss'>"+f+"</div></div></div>"+A);$(".menuBar").first().attr("id","tocBar");$(".transparent").remove();d()});$(document).on("click",".neueZeichenflaeche",function(){m();$(this).parent().after("<div class='zeichenflaeche' id='zeichnung'></div>"+A);bearbeitungAn=!0;D="zeichenflaeche";h=998;e=701;QreatorBezier.init("#zeichnung",998,701,"#menu",!0);d()});$(document).on("click",".markieren",function(){m();var a=
(new Date).getTime(),b=$(this).attr("lastClick");$(this).attr("lastClick",a);if(""==b||null==b)b=""+a;b=parseInt(b);if(1E3>a-b&&0==w)$(this).removeClass("aktiv"),$(this).addClass("bereich"),w=!0;else if($(this).hasClass("bereich")&&1==w)w=!1,$(this).removeClass("bereich"),$(this).removeClass("aktiv");else if(1!=w||$(this).hasClass("bereich"))$(this).toggleClass("aktiv");else{var c=-1,e=-1;$(this).addClass("endeBereich");$(this);var f=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("endeBereich")?
(e=f,$(this).removeClass("endeBereich")):$(this).hasClass("bereich")&&(c=f);f++});f=0;c<e?$(".markieren").not(".versteckt").each(function(){f>=c&&f<=e&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),$(this).addClass("aktiv"),w=!1);f++}):c>e&&$(".markieren").not(".versteckt").each(function(){f>=e&&f<=c&&($(this).removeClass("bereich"),$(this).removeClass("aktiv"),w=!1);f++})}d()});$("#markiereAlle").click(function(){m();w=!1;var a=0;$(".tshareElement").not(".versteckt").each(function(){a++});
var b=0;$(".markieren").not(".versteckt").each(function(){$(this).hasClass("aktiv")&&b++});var c=(new Date).getTime();b<a?($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").addClass("aktiv").removeClass("bereich");$(this).attr("lastClick",c)}),d()):($(".markieren").not(".versteckt").each(function(){$(this).removeClass("aktiv").removeClass("bereich")}),$(".wahl").each(function(){$(this).removeClass("komplett")}))});$("#auswahlLoeschen").click(function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlten Abschnitte inklusive aller Inhalte entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&
(m(),$(".markieren").each(function(){$(this).hasClass("aktiv")&&($(this).parent().prev().remove(),$(this).parent().remove())}))});$(document).on("click",".einfuegenClipboard",function(){$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><p>Bitte f\u00fcgen Sie den Inhalt des Clipboards in das Textfeld ein (z. B. per Strg+V) und dr\u00fccken Sie dann den Knopf \"Einf\u00fcgen\"</p><textarea id='clipboardExtern' required='required' ></textArea><button id='clipboardExternEinfuegen'>Einf\u00fcgen in Dokument</button><button id='abbrechen'>Abbrechen</button></div>");
$("#clipboardExtern").focus()});$(document).on("click","#clipboardExternEinfuegen",function(){var a=$("#clipboardExtern").val();a.startsWith("JEIF?EJFM?jmEHl\u00a7/87h78\u00a7FHLhl\u00a7F")?1==confirm("Sind Sie sicher, dass dieser Abschnitt eingef\u00fcgt werden soll? Dieser Schritt kann nicht mehr r\u00fcckg\u00e4ngig gemacht werden!")&&(m(),$("#einsetzen").after(a.substring(30,a.length)),$(".transparent").remove(),$("#einsetzen").remove(),d()):window.alert("Der Inhalt hat das falsche Format. Es k\u00f6nnen nur Inhalte aus Tshare eingef\u00fcgt werden.")});
$(document).on("click",".einfuegen",function(){m();$(this).parent().after(H);d()});$(document).on("click",".markdownEinfuegen",function(){m();$(this).parent().after("<div class='markdown tshareElement' id='markdown'></div>"+I);bearbeitungAn=!0;D="markdown";Textabschnitt.init("#markdown","#menu");d()});$(document).on("click",".changeSeparator",function(){m();var a=0,b=0,b=parseInt($(this).parent().prev(".tshareElement").attr("status"));if(void 0==b||isNaN(b))b=0;a=(b+1)%3;$(this).parent().prev(".datei").find("hr").each(function(){$(this).attr("class",
"");$(this).addClass("style_"+a)});$(this).parent().prev(".tshareElement").removeAttr("status");$(this).parent().prev(".tshareElement").attr("status",a);$(this).removeClass("statusFarbeKnopf_"+b);$(this).addClass("statusFarbeKnopf_"+a)});$(document).on("click",".bildEinfuegen",function(){m();$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='bildInput' type='file' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");
$("#bildInput").trigger("click")});this.saveObject=function(){for(var a=window.event.srcElement,b=a.previousSibling.getAttribute("data"),c=a.previousSibling.getAttribute("type"),a=a.previousSibling.getAttribute("name"),d=b.indexOf(","),b=atob(b.substring(d+1)),d=[],e=0;e<b.length;e+=512){for(var f=b.slice(e,e+512),h=Array(f.length),k=0;k<f.length;k++)h[k]=f.charCodeAt(k);f=new Uint8Array(h);d.push(f)}c=new Blob(d,{type:c});saveAs(c,a)};$(document).on("change","#bildInput",function(){var a=$("#bildInput")[0].files[0],
b=new FileReader;b.onload=function(b){$("#bildInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");var c=new Date,c=c.getDate()+"."+(c.getMonth()+1)+"."+c.getFullYear()+", "+c.getHours()+":"+("0"+c.getMinutes()).substr(-2)+":"+("0"+c.getSeconds()).substr(-2),c="|===\n.3+|icon:file-o[5x]|Dateiname|"+a.name.split(" ").join("_")+"\n|Gr\u00f6\u00dfe in Bytes|"+a.size+"|Einf\u00fcgezeitpunkt|"+c+"\n|===",d=Opal.hash2(["header_footer","attributes"],{header_footer:!1,
attributes:["icons=font"]}),c=Opal.Asciidoctor.$convert(c,d);$("#einsetzen").before("<div class='datei tshareElement'><div style='background-color: #f8f8f7'><object style='display:none' id='dataNeu' data='' name="+a.name.split(" ").join("_")+" type='"+a.type+"'></object><button onclick='saveObject()'>"+a.name.split(" ").join("_")+" herunterladen</button><div class='adoccss'>"+c+"</div></div></div>"+A);$("#dataNeu").attr("data",b.target.result);$("#dataNeu").removeAttr("id");$(".transparent").remove();
$("#einsetzen").remove()};b.readAsDataURL(a);d()});$(document).on("click",".downloadFile",function(){var a=$(this).attr("data"),a=Base64Binary.decodeArrayBuffer(a);new Blob([a],{type:$(this).attr("datatype")});saveAs([a],$(this).attr("filename"))});$(document).on("click",".pdf",function(){m();$(this).parent().after("<div id='einsetzen'></div>");$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='pdfInput' type='file' accept='application/pdf' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");
$("#pdfInput").trigger("click")});$(document).on("change","#pdfInput",function(){var a=$("#pdfInput")[0].files[0],b=new FileReader;b.onload=function(b){if(a.name.endsWith(".pdf")){var c=function(a){that.seitenZaehler++;var b=a.getViewport(1),b=a.getViewport(2*h/b.width),p=document.createElement("canvas");p.style.display="block";var t=p.getContext("2d");p.height=b.height;p.width=b.width;a=a.render({canvasContext:t,viewport:b});var m=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){m.call(this,
a);this.seitennummer=this.pageNumber;var b=this;a=t.getImageData(0,0,p.width,p.height);for(var c=0,d=a.data.length;c<d;c+=4){var l=a.data[c+1],n=a.data[c+2];255==a.data[c]&&255==l&&255==n&&(a.data[c+3]=0)}t.putImageData(a,0,0);r=new Image;r.onload=function(){f=p.width;k=p.height;that.zaehler++;QreatorBezier.insertImgAsSVG(this,h,e,"einsetzen"+b.seitennummer,A);console.log("Z\u00e4hler "+that.zaehler+" Seitennummer: "+b.seitennummer);that.zaehler==g&&($(".einsetzen").remove(),$(".transparent").remove())};
r.src=p.toDataURL()};d++;null!==l&&d<=g&&l.getPage(d).then(c)};$("#pdfInput").hide();$("#abbrechen").hide().after("<div id='ladeInfo'>Bitte warten ...</div>");PDFJS.disableWorker=!0;var d=1,g=0,l=null;this.bildLadeZaehler=this.seitenZaehler=this.zaehler=0;that=this;PDFJS.getDocument(b.target.result).then(function(a){l=a;g=a.numPages;for(var b=0;b<g;b++)$("#einsetzen").before("<div class='einsetzen' id='einsetzen"+(b+1)+"'></div>");$("#einsetzen").remove();a.getPage(1).then(c)})}};b.readAsDataURL(a);
d()});$(document).on("click",".zeichenflaecheKlick",function(){m();var a=$(this).html();$(this).removeClass("zeichenflaecheKlick");bearbeitungAn=!0;D="zeichenflaeche";var b=parseInt($(this).find("svg").attr("width")),c=parseInt($(this).find("svg").attr("height"));QreatorBezier.init(this,b,c,"#menu",!0);QreatorBezier.loadSVG(a)});$(document).on("click",".markdownText",function(){m();var a=$(this).find(".originaltext").text();$(this).parent().attr("id","markdown");$(this).parent().removeClass("adoccss");
bearbeitungAn=!0;D="markdown";Textabschnitt.init("#markdown","#menu",a)});$(document).on("change","#fileInput",function(){var a=$("#fileInput")[0].files[0],b=new FileReader;b.onload=function(a){a=a.target.result;var b=document.createElement("div");b.id="container";b.innerHTML=a;$("#content").html("<span id='firstMenu'><button class='einfuegen imageInsertInternal unten'></button><button class='einfuegenClipboard imagePaste unten'></button><button class='neueZeichenflaeche imageJournal unten' ></button><button class='pdf imagePDF unten'></button><button class='markdownEinfuegen imageNew unten'></button><button class='bildEinfuegen imageFotoNeu unten'></button></span>");
$(b).find(".tshareElement").each(function(){var a=$(this).attr("sb");if(null==a||""==a)a="@";var b=$(this).attr("ft");if(null==b||""==b)b="=";$(this).attr("tags");$(this).hasClass("zeichenflaeche")?($("#content").append("<div class='zeichenflaeche tshareElement zeichenflaecheKlick'>"+$(this).html()+"</div>"),$("#content").append(A)):$(this).hasClass("markdown")?($("#content").append("<div class='markdown tshareElement adoccss'>"+$(this).html()+"</div>"),$("#content").append(I)):$(this).hasClass("datei")?
($("#content").append("<div class='datei tshareElement'>"+$(this).html()+"</div>"),$("#content").append(A)):$(this).hasClass("toc")&&($("#content").append("<div id='toc' class='toc tshareElement'>"+$(this).html()+"</div>"),$("#content").append(A),$(".menuBar").last().attr("id","tocBar"));$(".ebenenAnzeige").last().attr("aktiv",a);$(".folientyp").last().val(b);a=B(a);$(".ebenenAnzeige").last().wrap("<span id='ersetzung'></span>");$("#ersetzung").html(a);$("#ersetzung .ebenenAnzeige").unwrap()});$(".transparent").remove();
d()};b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsText(a)});$(document).on("click","#laden",function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='fileInput' type='file' accept='text/html' required='required' ></input><button id='abbrechen'>Abbrechen</button><div id='parameter'></div></div>");$("#fileInput").trigger("click")});$(document).on("click","#bild",
function(){$("body").append("<div class='transparent'></div>");$(".transparent").append("<div class='fenster'><input id='imageInput' type='file' accept='image/*' required='required' ></input><button id='abbrechenBild'>Abbrechen</button><div id='parameter'></div></div>");$("#imageInput").trigger("click")});$(document).on("click","#abbrechen",function(){$(".transparent").remove();$("#einsetzen").remove()});$(document).on("click","#abbrechenBild",function(){QreatorBezier.loescheBild();$(".transparent").remove()});
$(document).on("mousemove",".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*f/h*b/100));isNaN(b)&&(b=parseInt(100*k/e*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());$(".fenster #breite").html(""+a);$(".fenster #hoehe").html(""+b);$(".fenster #xpos").html(""+c);$(".fenster #ypos").html(""+d)});$(document).on("change",".fenster .parameter",function(){var a=parseInt($("[typ='breite']").val()),
b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*f/h*b/100));isNaN(b)&&(b=parseInt(100*k/e*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(r,parseInt(c*h/100),parseInt(d*e/100),parseInt(a*h/100),parseInt(b*e/100),!1)});$(document).on("click","#bildEinfuegen",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=100*f/h*b/100);isNaN(b)&&(b=100*k/e*a/100);var c=parseInt($("[typ='x']").val()),
d=parseInt($("[typ='y']").val());QreatorBezier.ladeBild(r,parseInt(c*h/100),parseInt(d*e/100),parseInt(a*h/100),parseInt(b*e/100),!0);$(".transparent").remove()});$(document).on("click",".mittig",function(){var a=parseInt($("[typ='breite']").val()),b=parseInt($("[typ='hoehe']").val());isNaN(a)&&(a=parseInt(100*f/h*b/100));isNaN(b)&&(b=parseInt(100*k/e*a/100));var c=parseInt($("[typ='x']").val()),d=parseInt($("[typ='y']").val());switch($(this).attr("typ")){case "xmitte":c=parseInt(50-a/2);break;case "ymitte":d=
parseInt(50-b/2);break;case "links":c=0;break;case "rechts":c=100-a;break;case "oben":d=0;break;case "unten":d=100-b}$(".fenster #xpos").html(""+c);$("[typ='x']").val(c);$(".fenster #ypos").html(""+d);$("[typ='y']").val(d);QreatorBezier.ladeBild(r,parseInt(c*h/100),parseInt(d*e/100),parseInt(a*h/100),parseInt(b*e/100),!1)});$(document).on("change","#imageInput",function(){var a=$("#imageInput")[0].files[0],b=new FileReader;b.onload=function(b){a.name.endsWith(".pdf")?(console.log("PDF"),PDFJS.disableWorker=
!0,console.log(b.target.result),PDFJS.getDocument(b.target.result).then(function(a){a.getPage(1).then(function(a){var b=a.getViewport(2.5),c=document.createElement("canvas"),d=c.getContext("2d");c.height=b.height;c.width=b.width;a=a.render({canvasContext:d,viewport:b});var g=a._internalRenderTask.callback;a._internalRenderTask.callback=function(a){g.call(this,a);a=d.getImageData(0,0,c.width,c.height);for(var b=0,l=a.data.length;b<l;b+=4){var m=a.data[b+1],n=a.data[b+2];255==a.data[b]&&255==m&&255==
n&&(a.data[b+3]=0)}d.putImageData(a,0,0);r=new Image;r.onload=function(){f=c.width;k=c.height;q=!0;this.width>this.height&&(q=!1);1==q?(k=Math.min(e,this.height),k<e&&(k=e),f=this.width*k/this.height,f>h&&(k=k*h/f,f=h,q=!1)):(f=Math.min(h,this.width),f<h&&(f=h),k=this.height*f/this.width,k>e&&(f=f*e/k,k=e,q=!0));console.log(r.src);0==q?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*
f/h)+"'></input><span id='breite'>"+parseInt(100*f/h)+"</span>%, H\u00f6he:<span id='hoehe'>"+parseInt(100*k/e)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*k/e)+"'></input><span id='hoehe'>"+parseInt(100*k/e)+"</span>%, Breite: <span id='breite'>"+parseInt(100*f/h)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,f,k,!1)};r.src=c.toDataURL()}})})):
(r=new Image,r.onload=function(){k=f=0;q=!0;this.width>this.height&&(q=!1);1==q?(k=Math.min(e,this.height),k<e&&(k=e),f=this.width*k/this.height,f>h&&(k=k*h/f,f=h,q=!1)):(f=Math.min(h,this.width),f<h&&(f=h),k=this.height*f/this.width,k>e&&(f=f*e/k,k=e,q=!0));0==q?$(".fenster #parameter").html("<br>Breite (in Prozent von Abschnittsbreite): <input typ='breite' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*f/h)+"'></input><span id='breite'>"+parseInt(100*f/h)+"</span>%, H\u00f6he:<span id='hoehe'>"+
parseInt(100*k/e)+"</span>%"):$(".fenster #parameter").html("<br>H\u00f6he (in Prozent von Abschnittsh\u00f6he): <input typ='hoehe' class='parameter' type='range' min='0' max='100' value='"+parseInt(100*k/e)+"'></input><span id='hoehe'>"+parseInt(100*k/e)+"</span>%, Breite: <span id='breite'>"+parseInt(100*f/h)+"</span>%");$(".fenster #parameter").append("<br>x-Position links oben:  <input typ='x' class='parameter' type='range' min='0' max='100' value='0'></input><span id='xpos'>0</span><button class='mittig' typ='links'>linksb\u00fcndig</button><button class='mittig' typ='xmitte'>horizontal zentrieren</button><button class='mittig' typ='rechts'>rechtsb\u00fcndig</button>");
$(".fenster #parameter").append("<br>y-Position links oben:  <input typ='y' class='parameter' type='range' min='0' max='100' value='0'></input><span id='ypos'>0</span><button class='mittig' typ='oben'>oben b\u00fcndig</button><button class='mittig' typ='ymitte'>vertikal zentrieren</button><button class='mittig' typ='unten'>unten b\u00fcndig</button>");$(".fenster #parameter").append("<br><button id='bildEinfuegen'>Bild einf\u00fcgen</button>");QreatorBezier.ladeBild(r,0,0,f,k,!1)},r.src=b.target.result)};
b.onerror=function(a){console.log(a.target.error.name)};b.onprogress=function(a){console.log(a.loaded,a.total)};b.readAsDataURL(a)});$(document).on("click",".loeschen",function(){1==confirm("Sind Sie sicher, dass die gew\u00e4hlte Zeichenfl\u00e4che inklusive Inhalt entfernt werden soll? Dieser Schritt kann nicht r\u00fcckg\u00e4ngig gemacht werden!")&&(m(),$(this).parent().prev().remove(),$(this).parent().remove(),d())})});
